...
API_URL = "http://localhost:3000/api/index/options/NIFTY"
...
def render_rich_dashboard(ts, atm, spot, expiry_str, metrics, by_strike, last_hour_rows):
    ...
    console.print(Group(snap, per_strike_table(by_strike), hist))

def print_tabulate_snapshot(ts, atm, spot, expiry_str, metrics, last_hour_rows):
    headers = ["Time", "ATM", "Total OI", "Call OI", "Put OI", "Difference", "PCR", "Sentiment"]
    row = [
        ts.strftime("%H:%M:%S"),
        atm,
        metrics["total_oi"],
        metrics["total_call_oi"],
        metrics["total_put_oi"],
        metrics["diff"],
        metrics["pcr"],
        metrics["sentiment"],
    ]
    print(f"Snapshot | Spot: {spot:.2f} | ATM: {atm} | Exp: {expiry_str or 'N/A'}")
    ...
    print(f"Last 1 Hour (3-min) | Rows: {len(last_hour_rows)} | Exp: {expiry_str or 'N/A'}")
    print(tabulate(h_rows, headers=headers, tablefmt="github"))

def main(poll_seconds=ROLLING_INTERVAL_MIN * 60):
    console.rule("[bold]NIFTY Option Chain OI Monitor (Current Expiry, ATM ±3 strikes)")
