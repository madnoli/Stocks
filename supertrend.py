...
CONFIG = {
    ...
    "MIN_BARS_REQUIRED": 3,  # Reduced from 10 for early-day testing
    "MAX_MISSING_DATA_PCT": 30,  # Increased from 15 for leniency
...
def supertrend_kde_analysis(df, symbol):
    ...
    try:
        supertrend, direction = calculate_supertrend(
            df,
            CONFIG["INDICATOR_PERIODS"]["SUPERTREND_ATR"],
            CONFIG["INDICATOR_PERIODS"]["SUPERTREND_MULT"]
        )
        ...
        if len(close) >= 2 and len(supertrend) >= 2:
            ...
            prev_st = supertrend.iloc[-2]
            curr_st = supertrend.iloc[-1]
        ...
        kde_prob_bull = 0.0
        kde_prob_bear = 0.0
        supertrend_signal = 0.0
        status = "Normal"

        if bullish_break and not pd.isna(buy_vol_ratio.iloc[-1]):
            vol_list = supertrend_bullish_volumes[symbol]
            vol_list.append(float(buy_vol_ratio.iloc[-1]))
            if len(vol_list) > CONFIG["SUPERTREND_BINS"]:
                vol_list.pop(0)
            if len(vol_list) >= 10:
                x_points, y_cumsum = kde_probability(
                    vol_list,
                    CONFIG["SUPERTREND_BANDWIDTH"],
                    CONFIG["SUPERTREND_BINS"]
                )
                if x_points and y_cumsum:
                    ...
                    if vol_std > 0:
                        ...
                        if kde_prob_bull >= CONFIG["SUPERTREND_KDE_THRESHOLD"]:
                            supertrend_signal = 4.0
                            status = f"🚀 SUPERTREND BULL KDE {kde_prob_bull*100:.1f}%"
                        elif kde_prob_bull >= 0.80:
                            supertrend_signal = 2.5
                            status = f"⬆️ ST Bull {kde_prob_bull*100:.1f}%"
                        else:
                            supertrend_signal = 1.0
                            status = f"📈 ST Break {kde_prob_bull*100:.1f}%"

        elif bearish_break and not pd.isna(sell_vol_ratio.iloc[-1]):
            vol_list = supertrend_bearish_volumes[symbol]
            vol_list.append(float(sell_vol_ratio.iloc[-1]))
            if len(vol_list) > CONFIG["SUPERTREND_BINS"]:
                vol_list.pop(0)
            if len(vol_list) >= 10:
                x_points, y_cumsum = kde_probability(
                ...
                if x_points and y_cumsum:
                    ...
                    if vol_std > 0:
                        ...
                        if kde_prob_bear >= CONFIG["SUPERTREND_KDE_THRESHOLD"]:
                            supertrend_signal = -4.0
                            status = f"💥 SUPERTREND BEAR KDE {kde_prob_bear*100:.1f}%"
                        elif kde_prob_bear >= 0.80:
                            supertrend_signal = -2.5
                            status = f"⬇️ ST Bear {kde_prob_bear*100:.1f}%"
                        else:
                            supertrend_signal = -1.0
                            status = f"📉 ST Break {kde_prob_bear*100:.1f}%"

        elif len(direction) >= 1:
            current_dir = direction.iloc[-1]
            if current_dir < 0:
                supertrend_signal = 0.5
                status = "📈 ST Bullish Trend"
            elif current_dir > 0:
                supertrend_signal = -0.5
                status = "📉 ST Bearish Trend"
...
def oi_surge_improved(df, lookback=20):
    if df is None or len(df) < lookback + 5:
        return pd.Series(index=df.index if df is not None else [], dtype='float64')
    try:
        # Use OI if present, else derive a soft proxy from Volume for continuity
        oi_col = None
        for c in df.columns:
            cu = c.upper()
        ...
        if oi_col is None:
            # No OI: approximate via volume as a conservative proxy
            vol = pd.to_numeric(df["Volume"], errors="coerce").fillna(0)
            vma = vol.rolling(lookback, min_periods=8).mean()
            vsd = vol.rolling(lookback, min_periods=8).std(ddof=0)
            vsd = vsd.where(vsd > 0, vma * 0.10)
            vz = ((vol - vma) / vsd).clip(-5, 5)
            return vz.fillna(0)
        else:
            oi = pd.to_numeric(df[oi_col], errors="coerce").fillna(0)
            oima = oi.rolling(lookback, min_periods=8).mean()
            oisd = oi.rolling(lookback, min_periods=8).std(ddof=0)
            oisd = oisd.where(oisd > 0, oima * 0.10)
            oi_z = ((oi - oima) / oisd).clip(-5, 5)
            return oi_z.fillna(0)
    except Exception:
        return pd.Series(0, index=df.index)
    ...
    for tf_min, df in timeframe_dataframes.items():
        ...
        try:
            ...
            if len(volsurge) > 2:
                ...
                if currentsurge >= 2.5:
                    scores['VolumeSurge'] = 3.0 if pricechange >= 0.008 else -3.0
                    has_strong_conditions = True
        ...
        try:
            ...
            if price_up and vol_high and oi_active and momentum_strong and supertrend_strong:
                scores['CallBias'] = 5.0
                scores['PutBias'] = 0.0
                if "SuperTrend" not in oi_status:
                    oi_status = "Ultra Strong Call Setup"
                has_strong_conditions = True
            elif (not price_up) and vol_high and oi_active and momentum_strong and supertrend_strong:
                scores['PutBias'] = -5.0
                scores['CallBias'] = 0.0
                if "SuperTrend" not in oi_status:
                    oi_status = "Ultra Strong Put Setup"
                has_strong_conditions = True
                ...
                scores['CallBias'] = 2.0
                scores['PutBias'] = 0.0
                if oi_active and "SuperTrend" not in oi_status:
        ...
        # Remaining indicators as price-change heuristics
        remaining = [
            "ADX","VWAP","MACD","EMA","CMF","ADL","OBV","ATR","Bollinger","ROC",
            "Stochastic","CCI","MA","WWL","RelVol","VWAPRegime","OBVConfirm",
            "PriceVelocity","GammaSqueezeRisk","IVCrushRisk","OptionFlow"
        ]
...
def fetch_hist_for_symbol(session, limiter, symbol, bar_minutes, duration_label):
    try:
        limiter.acquire()
        bars = session.get_historic_data(
    ...
    with ThreadPoolExecutor(max_workers=CONFIG["MAX_WORKERS"]) as ex:
        ...
        for fut in it:
            ...
            if err:
                ...
                logger.debug(f"Fetch error for {symbol} {tf}min: {err}")
            else:
                if df is not None and not df.empty:
                    logger.debug(f"Fetched {len(df)} bars for {symbol} {tf}min")
                    symbol_to_tf_df[symbol][tf] = df
                else:
                    logger.debug(f"Empty DF for {symbol} {tf}min")
    ...
    for symbol, tf_map in symbol_to_tf_df.items():
        ...
        for tf, df in tf_map.items():
            ...
            if len(use_df) >= CONFIG["MIN_BARS_REQUIRED"]:
                cleaned[tf] = use_df
        logger.debug(f"{symbol}: cleaned TFs {list(cleaned.keys())}, total bars {sum(len(d) for d in cleaned.values())}")
...
def debug_fetch():
    if not tdhist_pool:
        console.print("[red]No sessions available for debug fetch.[/red]")
        return
    session = tdhist_pool[0]
    try:
        bars = session.get_historic_data("RELIANCE-EQ", duration="45 D", bar_size="5 min")
        df = pd.DataFrame(bars)
        norm_df = normalize_hist_df_clean(df, "RELIANCE-EQ", 5)
        console.print(f"[cyan]Raw bars: {len(bars) if bars else 0}[/cyan]")
        console.print(f"[cyan]Norm bars: {len(norm_df) if norm_df is not None else 0}[/cyan]")
        if norm_df is not None and not norm_df.empty:
            console.print(norm_df.tail())
    except Exception as e:
        console.print(f"[red]Debug fetch error: {e}[/red]")
