...
def get_live_cutoff_time():
    """LIVE MODE FIX: Get the cutoff time for live data - current completed 5-minute bar"""
    now_ist = datetime.now(IST)
    # Get the last completed 5-minute boundary
    minute = (now_ist.minute // 5) * 5
    cutoff = now_ist.replace(minute=minute, second=0, microsecond=0)

    # If we're exactly on the boundary, go back 5 minutes to get completed bar
...
# ======== CLEAN DATA FETCHING WITH LIVE MODE FIX ========
@retry(
...
def fetch_one_clean(symbol_orig, timeframe_minutes, limiter, hist):
    """LIVE MODE FIX: Clean fetch function with live data cutoff"""
    td_symbol = symbol_orig.replace("-EQ", "")

    bar_size = CONFIG["BAR_SIZE_MAP"].get(timeframe_minutes)

    # LIVE MODE FIX: Use different duration maps for live vs backtest
    if is_live_mode:
        duration = CONFIG["DURATION_MAP"].get(timeframe_minutes)
    else:
        duration = CONFIG["BACKTEST_DURATION_MAP"].get(timeframe_minutes)
    ...
    try:
        ...
        # LIVE MODE FIX: Apply cutoff time for real-time data
        if is_live_mode and df is not None:
            live_cutoff = get_live_cutoff_time()
            if hasattr(df.index, 'tz_convert'):
                df = df[df.index <= live_cutoff]
...
def prefetch_clean(stocks, max_workers=CONFIG["MAX_WORKERS"]):
    ...
    console.print(f"📡 Loading real-time data for [cyan]{len(valid_stocks)} symbols[/cyan]...")
...
def render_signals_clean(stock_data, scan_time_aware=None, show_neutral=False):
    """CLEAN: Enhanced rendering with essential display"""
    if not stock_data:
        console.print("❌ [red]No real-time data available[/red]")
        return 0

    if scan_time_aware is None:
        scan_time_aware = datetime.now(IST)

    signals = []
    strong_signals_count = 0

    for symbol, timeframe_data in stock_data.items():
        if not timeframe_data:
            continue

        try:
            filtered_tfs = filter_timeframe_data(symbol, timeframe_data, scan_time_aware)
            if not filtered_tfs:
                continue

            signal, score, oi_status, action, priority = analyze_signals_enhanced_clean(filtered_tfs, symbol)

            if abs(score) >= CONFIG["MIN_SIGNAL_THRESHOLD"] or show_neutral:
                signals.append({
                    'symbol': symbol,
                    'signal': signal,
                    'score': score,
                    ...
                    'action': action,
                    'priority': priority,
                    'timeframes': len(filtered_tfs)
                })

                if abs(score) >= 20:  # Strong signals
    ...
    # Create rich table
    table = Table(title=f"🎯 Option Signals | {scan_time_aware.strftime('%H:%M:%S IST')}", box=box.ROUNDED)
    table.add_column("Symbol", style="cyan", width=12)
    table.add_column("Signal", style="bold", width=25)
    table.add_column("Score", justify="right", width=8)
    table.add_column("OI Status", style="yellow", width=18)
    table.add_column("Option Action", style="bold green", width=20)
    table.add_column("TFs", justify="center", width=4)

    for sig in signals[:15]:  # Show top 15 signals
        score_color = "green" if sig['score'] > 0 else "red"
        score_str = f"[{score_color}]{sig['score']:.1f}[/{score_color}]"
    ...
    console.print(table)
    console.print(f"⚡ [bold green]{strong_signals_count} strong signals detected[/bold green]")

    return len(signals)

def export_to_csv(signals, filename_prefix="option_signals"):
    ...
    try:
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['Timestamp', 'Symbol', 'Signal', 'Score', 'OI_Status', 'Option_Action', 'Priority', 'Timeframes']
            ...
            for sig in signals:
                writer.writerow({
                    ...
                    'OI_Status': sig['oi_status'],
                    'Option_Action': sig['action'],
                    'Priority': sig['priority'],
                    'Timeframes': sig['timeframes']
                })

        console.print(f"💾 [green]Signals exported to {filename}[/green]")
        return filename
    except Exception as e:
...
def send_live_alerts(signals):
    ...
    urgent_signals = [s for s in signals if s['priority'] in ['URGENT', 'HIGH']]

    if urgent_signals:
        console.print(f"🚨 [bold red]{len(urgent_signals)} URGENT ALERTS![/bold red]")
        for sig in urgent_signals:
            console.print(f"⚡ {sig['symbol']}: {sig['action']} | Score: {sig['score']:.1f}")
...
def load_stock_list(filepath):
    ...
    try:
        ...
        console.print(f"📋 Loaded [cyan]{len(stocks)} stock symbols[/cyan]")
        return stocks

    except Exception as e:
        console.print(f"[red]❌ Error loading stocks: {e}[/red]")
        return []

# ======== LIVE MODE WITH 5-MINUTE BOUNDARY SCANNING ========
def run_live_mode_clean(stocks):
    """LIVE MODE: Enhanced with 5-minute boundary scanning and live data cutoff"""
    global previous_scores, last_bull_symbols, last_bear_symbols, is_live_mode

    is_live_mode = True  # Set the live mode flag
    ...
    if now_ist < market_start:
        ...
        console.print("⏳ Waiting until market opens...")
...
def run_realtime_scan(stocks, scan_time_aware):
    """Run a single real-time scan"""
    try:
        # LIVE MODE: Fetch data with live cutoff applied
        stock_data = prefetch_clean(stocks)
        ...
        # Render signals
        signals_count = render_signals_clean(stock_data, scan_time_aware)

        # Export if there are signals
        if signals_count > 0:
            signals = []
            for symbol, timeframe_data in stock_data.items():
                try:
                    ...
                    if filtered_tfs:
                        signal, score, oi_status, action, priority = analyze_signals_enhanced_clean(filtered_tfs, symbol)
                        if abs(score) >= CONFIG["MIN_SIGNAL_THRESHOLD"]:
                            signals.append({
                                'symbol': symbol, 'signal': signal, 'score': score,
                                'oi_status': oi_status, 'action': action, 'priority': priority,
                                'timeframes': len(filtered_tfs)
                            })
                except:
                    continue
            ...
            if signals:
                export_to_csv(signals, "live_signals")
                send_live_alerts(signals)
...
# ======== BACKTEST MODE ========
def run_backtest_clean(backtest_date_str, stocks):
    """CLEAN: Enhanced backtesting"""
    global is_live_mode
    is_live_mode = False  # Set backtest mode flag

    try:
        backtest_date = datetime.strptime(backtest_date_str, "%Y-%m-%d")
        backtest_date_ist = IST.localize(backtest_date.replace(hour=9, minute=15))
    except ValueError:
        console.print("[red]❌ Invalid date format. Use YYYY-MM-DD[/red]")
        return

    console.print(f"📅 [cyan]Backtesting {backtest_date_str}[/cyan]")

    checkpoints = day_checkpoints_ist(backtest_date_ist)

    console.print(f"⏱️  Running [yellow]{len(checkpoints)} scans[/yellow] for the day")

    for i, checkpoint in enumerate(checkpoints, 1):
        console.print(f"\n🔄 [bold cyan]Scan {i}/{len(checkpoints)}[/bold cyan] | {checkpoint.strftime('%H:%M IST')}")

        # Fetch data for backtest
        stock_data = prefetch_clean(stocks)

        if stock_data:
            signals_found = render_signals_clean(stock_data, checkpoint)
            if signals_found:
                console.print(f"⚡ Found [green]{signals_found}[/green] signals")
            else:
                console.print("📊 [yellow]No signals at this time[/yellow]")
        else:
