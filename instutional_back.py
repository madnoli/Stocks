# Enhanced Scanner for Option Buyers (v24-Compatible, TrueData, 5/15/30/60/1D, unified indicators,
# multi-session 10 rps, tz-aware, backdated replay with per-minute prints, single progress bar)
...
# Live settings (defaults; can be overridden via CLI)
...
# ---------------- CLI args (NEW) ----------------
def parse_args():
    parser = argparse.ArgumentParser(description="Enhanced Scanner for Option Buyers (live + backdated replay).")
    parser.add_argument("--asof", type=str, default=None, help="Run replay for this date (YYYY-MM-DD) in IST.")  # YYYY-MM-DD enforced by code
    parser.add_argument("--step-mins", type=int, default=5, help="Replay step minutes, e.g., 5 prints at 09:15, 09:20, …")
    parser.add_argument("--refresh-mins", type=int, default=0, help="If >0, refetch cached data every N minutes during replay.")
    parser.add_argument("--loop", action="store_true", help="Live mode only; ignored for replay. Keeps scanning.")
    parser.add_argument("--market-start", type=str, default=os.getenv("MARKET_START", MARKET_START), help="Session start HH:MM IST.")
    parser.add_argument("--market-end", type=str, default=os.getenv("MARKET_END", MARKET_END), help="Session end HH:MM IST.")
    parser.add_argument("--scan-seconds", type=int, default=SCAN_SECONDS, help="Seconds between scans in live mode.")
    return parser.parse_args()  # argparse supports custom types for date parsing patterns [web:81][web:42]

def ist_session_bounds_for_date(yyyy_mm_dd: str, market_start: str, market_end: str):
    d = datetime.strptime(yyyy_mm_dd, "%Y-%m-%d")
    sh, sm = map(int, market_start.split(":"))
    eh, em = map(int, market_end.split(":"))
    start = IST.localize(datetime(d.year, d.month, d.day, sh, sm, 0))
    end   = IST.localize(datetime(d.year, d.month, d.day, eh, em, 0))
    return start, end  # tz-aware IST bounds for slicing [web:87]

def build_replay_points(start_ist: datetime, end_ist: datetime, step_mins: int):
    pts = []
    t = start_ist
    while t <= end_ist:
        pts.append(t)
        t = t + timedelta(minutes=step_mins)
    return pts  # inclusive stepping over the IST session [web:62]
...
def build_sessions():
    ...
    per_sess_rate = 10.0 / len(pool)  # ~10 rps split across sessions
...
# ---------------- Main loop (live + backdated replay) ----------------
def live_market_scanner_top20():
    args = parse_args()
    global SCAN_SECONDS, MARKET_START, MARKET_END
    SCAN_SECONDS = args.scan_seconds
    MARKET_START = args.market_start
    MARKET_END = args.market_end

    backdate_mode = args.asof is not None

    logger.info("Starting market scanner for OPTION BUYERS (IST)…")
    if backdate_mode:
        logger.info("Mode: Backdated replay.")
    else:
        logger.info("Mode: Live Continuous Scanning." if args.loop else "Mode: Live One-pass then sleep loop.")
    ...
    if backdate_mode:
        output_filename = args.asof + "_options_scan_results.txt"
    else:
        output_filename = datetime.now(IST).strftime("%Y-%m-%d") + "_options_scan_results.txt"

    # ---------------- Backdated replay branch ----------------
    if backdate_mode:
        start_ist, end_ist = ist_session_bounds_for_date(args.asof, MARKET_START, MARKET_END)
        replay_points = build_replay_points(start_ist, end_ist, args.step_mins)

        last_refresh_t = start_ist
        previous_scores = {}

        for tp in replay_points:
            now_ist = tp  # simulated clock
            # Optional periodic refresh (historical data is static; keep 0 unless needed)
            if args.refresh_mins > 0 and (tp - last_refresh_t).total_seconds() >= args.refresh_mins * 60:
                logger.info(f"[{tp.strftime('%H:%M:%S')}] Refreshing data for {len(stock_multi_data)} stocks...")
            ...
            # Build signals for this replayed minute
        ...
        logger.info("Backdate replay complete. Exiting.")
        return
