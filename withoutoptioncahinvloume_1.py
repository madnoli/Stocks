...
# ENHANCEMENT: ADDED VolumeX (Volume/SMA 20 Multiple) FOR EXTREME SURGE DETECTION
# ==============================================================================
...
class Config:
    ...
    INDICATOR_WEIGHTS = {
        ...
        "VolumeSurge": 3.5, "OBV": 2.5, "CMF": 2.5, "RelVol": 2.5, "VolumeX": 4.0, # ADDED VOLUME X WEIGHT
...
def volume_sma_multiple(df, period=20):
    """Calculate the ratio of current Volume to its SMA 20 (VolumeX)"""
    if df.empty or len(df) < period:
        # Return 0 or NaN for insufficient data
        return pd.Series(dtype='float64', index=df.index).fillna(0)
    
    vol_sma = df["Volume"].rolling(period).mean()
    # Calculate the ratio (VolumeX)
    volume_x = df["Volume"] / vol_sma.replace(0, np.nan)
    return volume_x.fillna(0)
# ---------------------------
...
def calculate_technical_indicator_scores(df):
    ...
    try:
        ...
        # Enhanced Relative Volume (RelVol) is already close, but let's add VolumeX explicitly
        # Calculate VolumeX (Volume/SMA 20)
        vol_x_series = volume_sma_multiple(df, period=20)
        if not vol_x_series.empty and len(vol_x_series) > 0:
            vol_x_val = vol_x_series.iloc[-1]
            
            # Scoring logic for VolumeX (High weight for extreme surge)
            if vol_x_val >= 3.0:
                scores['VolumeX'] = 4.0 # Extreme surge gets very high score
            elif vol_x_val >= 2.5:
                scores['VolumeX'] = 3.5 # Meets the 2.5x condition with a strong score
            elif vol_x_val >= 2.0:
                scores['VolumeX'] = 2.0
            elif vol_x_val <= 0.5:
                scores['VolumeX'] = -2.0 # Low volume gets a penalty
            else:
                scores['VolumeX'] = 0.0
            
            # --- Store the raw value for the final display column ---
            df['_VolumeX'] = vol_x_series # Store in DataFrame for later extraction
            ...
            for indicator, ind_weight in Config.INDICATOR_WEIGHTS.items():
                if indicator in indicator_scores:
                    belongs_to_group = (
                        ...
                        # ADDED 'VolumeX' to the Volume group check
                        (group == 'Volume' and any(term in indicator for term in ['Vol', 'OBV', 'CMF', 'VolumeX'])) or
...
def run_ultimate_scan_at_time(time_point_aware, stocks, market_regime, is_live=False):
    ...
    for truedata_symbol, timeframe_data in stock_multi_data.items():
        ...
        if abs(score) >= Config.SCORE_THRESHOLD_MIN:
            ...
            main_tf_data = filtered_timeframes.get(5, filtered_timeframes.get(15, filtered_timeframes.get(30, list(filtered_timeframes.values())[0])))
            ...
            if main_tf_data is not None:
                ...
                # --- NEW: Extract VolumeX ---
                # Check if the scoring function added the '_VolumeX' column
                volume_x_val = main_tf_data.get('_VolumeX', pd.Series())
                if not volume_x_val.empty:
                    vol_x = volume_x_val.iloc[-1]
                else:
                    # Recalculate if not found (safer for backtesting)
                    vol_x = volume_sma_multiple(main_tf_data, period=20).iloc[-1] if len(main_tf_data) >= 20 else 0.0
            else:
                oi_vol_data = extract_5min_volume_oi_data(None, clean_symbol, time_point_aware, is_live=is_live)
                vol_x = 0.0
            ...
            result = {
                ...
                **oi_vol_data,
                ...
                # --- NEW: VolumeX for display/filtering ---
                'volumeX': vol_x,
...
def create_ultimate_option_table(data, title, new_stocks=None, show_time=None):
    ...
    if RICH_AVAILABLE:
        ...
        table.add_column("VolX", style="red", width=5, justify="right") # ADDED VolumeX Column
        ...
        for item in data:
            ...
            # --- NEW: Format VolumeX ---
            vol_x_val = item.get('volumeX', 0.0)
            vol_x_display = f"{vol_x_val:.1f}X"
            
            # Color coding for VolumeX to highlight 2.5x
            vol_x_style = f"[bold bright_red]{vol_x_display}[/bold bright_red]" if vol_x_val >= 2.5 else f"[red]{vol_x_display}[/red]"
            # ---------------------------
            ...
            table.add_row(
                ...
                vol_display,
                oi_display,
                vol_x_style, # ADDED VolumeX Row Entry
        ...
        print_colored("="*106, Colors.BLUE) # Adjusted length
        # ADDED VolX to header
        header = f"{'Stock':<8} | {'Signal':<16} | {'Score':>6} | {'Vol%':>6} | {'OI%':>6} | {'VolX':>5} | {'Flow':<14} | {'P.Chg%':>7} | {'Qual':>6}"
        ...
        for item in data:
            ...
            # ADDED VolX to row
            row = f"{symbol:<8} | {item['signal']:<16} | {item['score']:>6.1f} | {vol_str:>6} | {oi_str:>6} | {vol_x_str:>5} | {item.get('flow', 'Unknown'):<14} | {price_str:>7} | {quality:>6.1f}"
...
def create_ultimate_summary_panel(signals):
    ...
    # --- NEW: VolumeX analysis ---
    extreme_volume = len([s for s in signals if s.get('volumeX', 0.0) >= 2.5])
    # -----------------------------
    ...
    if RICH_AVAILABLE:
        # Updated summary text with VolumeX
        summary_text = f"""
...
[red]Extreme Volume (VolX >= 2.5): {extreme_volume}[/red]
...
[blue]Avg Vol Chg: {avg_vol_change:.1f}%[/blue]
        ...
        print(f"Extreme Volume (VolX >= 2.5): {extreme_volume}") # Added VolumeX
...
if __name__ == "__main__":
    try:
        ...
        print("  🆕 **VolumeX (Volume/SMA 20) for Extreme Surge**") # Highlighted new feature
