...
# ======== UNIFIED FILTER WITH CONSISTENT LOGIC ========
def filter_timeframe_data_to_boundary_unified(symbol, timeframe_data, cutoff_time_aware, mode="live"):
    """Unified filtering logic for both backtest and live modes"""
    filtered_timeframes = {}
    ...
    for tf, df in timeframe_data.items():
        ...
        try:
            ...
            # **KEY FIX**: Use consistent filtering logic
            if mode == "backtest":
                # For backtest: include data UP TO cutoff (<=)
                mask = df.index <= cutoff_time_aware
            else:
                # For live: include data BEFORE cutoff (<) to ensure completion
                mask = df.index < cutoff_time_aware
...
def print_clean_banner():
    console.print("\n" + "="*70, style="bold blue")
    console.print("🎯 [bold cyan]OPTION SIGNAL SCANNER v3.1 - UNIFIED BOUNDARIES[/bold cyan] 🎯", justify="center")
    console.print("="*70, style="bold blue")
    console.print(f"🕐 [bold white]{datetime.now(IST).strftime('%H:%M:%S IST')}[/bold white] | Mode: [green]UNIFIED 5-MIN BOUNDARY ANALYSIS[/green]")
    console.print("="*70, style="bold blue")
...
# ======== UNIFIED BACKTEST ========
def run_backtest_unified(day_str: str, stocks):
    """Updated backtest with unified boundary logic"""
    day_date = datetime.strptime(day_str, "%Y-%m-%d")
    console.print(f"📊 [bold cyan]Backtesting {day_str}[/bold cyan] - [yellow]{len(stocks)} symbols[/yellow] - UNIFIED LOGIC")
    ...
    output_filename = day_date.strftime("%Y-%m-%d") + "_signals_unified.csv"
    ...
    for i, checkpoint_ts in enumerate(checkpoints):
        if (i + 1) % 20 == 0:
            console.print(f"📈 Progress: [cyan]{i+1}/{len(checkpoints)}[/cyan] | Boundary: [yellow]{checkpoint_ts.strftime('%H:%M')}[/yellow]")

        # Use unified cutoff logic
        analysis_cutoff = get_unified_analysis_cutoff("backtest", checkpoint_ts)
        ...
        for symbol, timeframe_data in stock_multi_data.items():
            ...
            filtered_timeframes = filter_timeframe_data_to_boundary_unified(
                clean_symbol, timeframe_data, analysis_cutoff, "backtest"
            )
        ...
        if top_bullish or top_bearish:
            ...
            export_to_csv(analysis_cutoff, top_bullish, top_bearish, output_filename)

    # Final summary
    total_signals = sum(performance_metrics.values())
    console.print(f"\n📈 [bold green]UNIFIED BACKTEST COMPLETE[/bold green]")
...
# ======== UNIFIED LIVE MODE ========
def run_live_mode_unified(stocks):
    """Updated live mode with unified boundary logic"""
    console.print("🚀 [bold red]LIVE MODE - UNIFIED 5-MINUTE BOUNDARIES[/bold red]")
    ...
    live_signals_file = f"live_signals_unified_{datetime.now(IST).strftime('%Y%m%d')}.csv"
    try:
    ...
    console.print(f"💾 Unified signals: [yellow]{live_signals_file}[/yellow]")
    ...
    try:
        while (market_start <= datetime.now(IST) <= market_end):
            ...
            # Analyze each symbol with unified boundary cutoff
            for symbol, timeframe_data in stock_multi_data.items():
                clean_symbol = symbol.replace('-EQ', '')

                # Use unified filtering
                filtered_timeframes = filter_timeframe_data_to_boundary_unified(
                    clean_symbol, timeframe_data, analysis_cutoff, "live"
                )
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(description="Option Scanner - Unified Production")
        ...
        if args.test_oi:
            ...
            run_live_mode_unified(stocks)

        elif args.backtest_date:
            try:
                datetime.strptime(args.backtest_date, "%Y-%m-%d")
                run_backtest_unified(args.backtest_date, stocks)
            except ValueError:
                console.print("[red]❌ Invalid date format. Use YYYY-MM-DD[/red]")
        else:
            console.print("\n🎯 [bold green]Option Scanner - Unified Production[/bold green]")
            console.print("[cyan]Features:[/cyan]")
            console.print("  ✅ Unified 5-minute boundary analysis")
            console.print("  ✅ Enhanced option buyer logic")
            console.print("  ✅ Consistent backtest-live results")
            console.print("  ✅ OI detection and institutional flow")
            console.print("\n[cyan]Usage:[/cyan]")
            console.print("  [yellow]python scanner_unified.py --backtest-date 2025-09-30[/yellow]")
            console.print("  [yellow]python scanner_unified.py --live[/yellow]")
