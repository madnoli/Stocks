# ==============================================================================
# ULTIMATE OPTION BUYER SCANNER v5.0 - ENHANCED FOR REAL OPTION TRADING - COMPLETE WITH ALL DATETIME FIXES
...
class Config:
    ...
    ATM_STRIKE_RANGE = 3
    STRONG_VOL_SURGE_RATIO = 2.5
    PCR_STRONG_BULL = 0.6
    PCR_STRONG_BEAR = 1.5
    IV_HIGH_THRESHOLD = 35.0
    ...
    INDICATOR_WEIGHTS = {
        ...
        "Option_Volume_Surge": 3.5,
        "ATM_Dominance": 3.0,
        "IV_Skew_Signal": 2.5,       # Implied volatility skew
        "Liquidity_Score": 3.0,
        "Bid_Ask_Spread": 2.5,
        "Max_Pain_Distance": 2.0,
        "Volume_Profile": 2.2,
    }
...
class LocalhostOptionChainAPI:
    ...
    def calculate_liquidity_score(self, total_oi, total_vol, strikes_data):
        """v5.0 ENHANCED"""
        try:
            oi_score = min(100, (total_oi / Config.MIN_TOTAL_OI) * 50)
            vol_score = min(100, (total_vol / Config.MIN_TOTAL_VOL) * 50)
            active_strikes = sum(1 for s in strikes_data if (s["ce_oi"] + s["pe_oi"]) > 100)
            strike_score = min(100, active_strikes * 10)
            atm_vol_sum = sum(s["ce_vol"] + s["pe_vol"] for s in strikes_data[:3])
            atm_vol_score = min(100, (atm_vol_sum / Config.MIN_ATM_VOLUME) * 20)
            liquidity_score = (oi_score * 0.3) + (vol_score * 0.3) + (strike_score * 0.2) + (atm_vol_score * 0.2)
    ...
    def calculate_pcvr(self, ce_vol, pe_vol):
        if ce_vol == 0:
            return float('inf') if pe_vol > 0 else 1.0
        return pe_vol / ce_vol

    def calculate_bid_ask_spread(self, strikes_data, atm_strike):
        try:
            spreads = []
            for strike_data in strikes_data:
                if abs(strike_data['strike'] - atm_strike) <= 10:
                    ce_bid = strike_data.get('ce_bid', 0)
                    ce_ask = strike_data.get('ce_ask', 0)
                    pe_bid = strike_data.get('pe_bid', 0)
                    pe_ask = strike_data.get('pe_ask', 0)
                    if ce_bid > 0 and ce_ask > ce_bid:
                        spreads.append(((ce_ask - ce_bid) / ce_bid) * 100)
                    if pe_bid > 0 and pe_ask > pe_bid:
                        spreads.append(((pe_ask - pe_bid) / pe_bid) * 100)
            return sum(spreads) / len(spreads) if spreads else 0
        except:
            return 0

    def calculate_max_pain(self, strikes_data):
        try:
            max_oi = 0
            max_pain_strike = 0
            for strike_data in strikes_data:
    ...
    def calculate_volume_surge_vs_history(self, symbol, current_volume):
        global historical_volume_cache
        try:
            if symbol not in historical_volume_cache:
                historical_volume_cache[symbol] = []
            historical_volume_cache[symbol].append(current_volume)
            if len(historical_volume_cache[symbol]) > 20:
                historical_volume_cache[symbol] = historical_volume_cache[symbol][-20:]
            if len(historical_volume_cache[symbol]) >= 5:
                avg_volume = sum(historical_volume_cache[symbol][:-1]) / (len(historical_volume_cache[symbol]) - 1)
                if avg_volume > 0:
...
def calculate_option_chain_scores(option_chain_data):
    ...
    # v5.0 NEW Scoring
    pcvr = option_chain_data.get("PCVR", 1.0)
    if pcvr is not None and pcvr != float('inf'):
        if pcvr < 0.5:
            scores['PCVR_Signal'] = 3.0
        elif pcvr < 0.8:
            scores['PCVR_Signal'] = 1.5
        elif pcvr > 2.0:
            scores['PCVR_Signal'] = -3.0
        elif pcvr > 1.5:
            scores['PCVR_Signal'] = -1.5

    bid_ask_spread = option_chain_data.get("Bid_Ask_Spread", 0)
    if bid_ask_spread > 0:
        if bid_ask_spread < 1.0:
            scores['Bid_Ask_Spread'] = 2.5
        elif bid_ask_spread < Config.MAX_BID_ASK_SPREAD:
            scores['Bid_Ask_Spread'] = 1.0
        elif bid_ask_spread > 5.0:
            scores['Bid_Ask_Spread'] = -2.5

    max_pain_distance = option_chain_data.get("Max_Pain_Distance", 0)
    if max_pain_distance > 0:
        underlying = option_chain_data.get("Price", 0)
        if underlying > 0:
            distance_pct = (max_pain_distance / underlying) * 100
            if distance_pct < 2:
                scores['Max_Pain_Distance'] = 1.0
            elif distance_pct > 5:
                scores['Max_Pain_Distance'] = -1.5

    volume_surge_ratio = option_chain_data.get("Volume_Surge_Ratio", 1.0)
    if volume_surge_ratio >= Config.STRONG_VOL_SURGE_RATIO:
        scores['Volume_Profile'] = 2.2
    elif volume_surge_ratio >= Config.MIN_VOL_SURGE_RATIO:
        scores['Volume_Profile'] = 1.0
    elif volume_surge_ratio < 0.7:
        scores['Volume_Profile'] = -1.0
            else:
