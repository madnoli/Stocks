# Final Trading Signal Script - v26 LIVE
# TrueData, 5/15/30/60/1D, lighter durations, all-signal ranking, tz-aware, single prefetch progress bar
...
# Lighter intraday durations; daily retained for context
DURATION_MAP = {5: "15 D", 15: "20 D", 30: "30 D", 60: "60 D", 1440: "365 D"}
...
# Optional test mode (single scan then exit)
...
def prefetch_all(stocks, max_workers=64):
    ...
    # Single tqdm bar instance; no separate rps thread
    with tqdm(total=total_calls, desc="Prefetching Data", ncols=100) as bar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for fut in as_completed(futures):
                ...
                # Inline rps postfix update every 20 calls
                with api_calls_lock:
                    curr_calls = api_calls_done
                now_ts = perf_counter()
                if curr_calls - last_calls >= 20:
                    elapsed = max(1e-6, now_ts - last_ts)
                    rps = (curr_calls - last_calls) / elapsed
                    bar.set_postfix_str(f"rps={rps:.0f}")
...
def append_table_output(file_path, scan_time_str, bullish_list, bearish_list):
    with open(file_path, "a", encoding="utf-8") as f:
        ...
        f.write("-" * 92 + "\n")
        if not bullish_list:
            f.write("No bullish signals found.\n")
        for r in bullish_list:
            ...
            if isinstance(change_val, (int, float, np.floating)):
                ...
                change_str = "NA"
            action = "Consider Long" if r['score'] >= 0 else "Consider Short"
        ...
        f.write("-" * 92 + "\n")
        if not bearish_list:
            f.write("No bearish signals found.\n")
...
def live_market_scanner():
    ...
    # Initial bulk prefetch to seed caches (single progress bar here only)
    ...
    while True:
        ...
        for symbol, timeframe_data in stock_multi_data.items():
            ...
            # All-signal ranking: direction from score sign
        ...
        # Rank: bullish by highest positive scores; bearish by most negative
        bullish_all = sorted([r for r in scored_rows if r['score'] >= 0], key=lambda x: x['score'], reverse=True)
        bearish_all = sorted([r for r in scored_rows if r['score'] < 0], key=lambda x: x['score'])
        top_bullish = bullish_all[:15]
        top_bearish = bearish_all[:15]
        ...
        # Bullish table
        print(f"| {'Top 15 Bullish (all signals)':<88} |")
        print("-"*92)
        if not top_bullish:
            print("| None".ljust(91) + " |")
        else:
            ...
            for result in top_bullish:
                ...
                colored_signal = f"{GREEN}{signal_text:<18}{RESET}" if result['score'] >= 0 else f"{RED}{signal_text:<18}{RESET}"
                action = f"{GREEN}Consider Long{RESET}" if result['score'] >= 0 else f"{RED}Consider Short{RESET}"

                print(f"| {result['symbol']:<15} | {colored_signal} | {result['score']:>7.2f} | {change_str:>{padding}} | {result['trend']:<10} | {action:<29} |")

        # Bearish table
        print("-"*92)
        print(f"| {'Top 15 Bearish (all signals)':<88} |")
        print("-"*92)
        if not top_bearish:
            print("| None".ljust(91) + " |")
        else:
