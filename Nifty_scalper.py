...
# -------------------------------
# Option Buyers Bias + Fast Scalper Mode
# -------------------------------
def option_buyer_bias_panel(
    ...
    # ----------------------------------
    # Primary Bias (swing-ish intraday)
    # ----------------------------------
    ...
    def scalp_flag(note, iv_trend, price_chg, vwap_ok, sentiment_ok, ltp):
        strong_move = abs(price_chg) >= max(3, ltp * 0.01)   # at least 3 pts or ~1%
        return (
            note == "Long Buildup"
            and iv_trend == "Rising"
            and strong_move
    ...
    scalper_lines = []
    if scalp_ce:
        scalper_lines.append(
            "⚡ [bold green]Scalp CE[/bold green]: target 5–20 pts, SL 5–10 pts, "
            "book quickly (1–3 candles)."
        )
    if scalp_pe:
        scalper_lines.append(
            "⚡ [bold green]Scalp PE[/bold green]: target 5–20 pts, SL 5–10 pts, "
            "book quickly (1–3 candles)."
        )
    if not scalper_lines:
        scalper_lines.append(
            "No high-conviction scalper setup right now (wait for clean long buildup + IV spike)."
        )

    # Expiry scalping mode message
    mode_line = ""
    if is_expiry_day:
        mode_line = "[bold magenta]Expiry Fast Scalper Mode:[/] focus on 5–10 pt quick moves.\n"
        if ts.hour >= 14 and ts.minute >= 30:
            mode_line += (
                "[red]Post 14:30 on expiry: only ultra-quick scalps, avoid holding for more than a few minutes.[/red]\n"
            )
...
def main(poll_seconds=ROLLING_INTERVAL_MIN * 60):
    console.rule(
        "[bold]NIFTY OI Monitor + Fast Scalper Mode (5–20 pts)"
    )
