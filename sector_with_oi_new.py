...
ENHANCED_INDICATOR_WEIGHTS = {
    ...
    "ATR": 1.8, # Volatility for option premium (increased for option buyers)
...
# =========================
TIMEFRAME_WEIGHTS = {
    5: 3.2, # Increased for quick moves (option buyers focus)
...
# =========================
def normalize_indicator_value(indicator_name, value, close=None):
    ...
    try:
        if indicator_name == "RSI":
            ...
            if close is not None and close != 0:
                ...
                # Favor higher volatility for option buyers
                return min(100, max(0, 50 + (atr_percent - 2) * 10))  # e.g., 2% ATR -> 50, 3% -> 60, etc.
            else:
                return 50
...
class EnhancedOptionBuyerScanner:
    ...
    def calculate_option_buyer_signals(self, symbol, timeframes_data):
        ...
        try:
                    ...
                    if name in indicators and indicators[name] is not None:
                        ...
                        if has_data:
                            ...
                            if name in ("MA", "EMA", "VWAP"):
                                ...
                                norm_score = normalize_indicator_value(name, latest_val, current_price if name == "ATR" else None)
            ...
            if base_score >= 80:  # Lowered threshold for Strong Call Buy
                return "Strong Call Buy", base_score
            elif base_score >= 70:  # Lowered for Call Buy
                return "Call Buy", base_score
            elif base_score >= 55:  # Lowered for Moderate Call
                return "Moderate Call", base_score
            elif base_score <= 20:  # Raised (made more sensitive) for Strong Put Buy
                return "Strong Put Buy", base_score
            elif base_score <= 30:  # Raised for Put Buy
                return "Put Buy", base_score
            elif base_score <= 45:  # Raised for Moderate Put
