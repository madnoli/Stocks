...
# MISSING FUNCTION: run_backtest_clean
def run_backtest_clean(day_str: str, stocks):
    ...
    # Get time checkpoints for the day
    ...
    for i, as_of_ts in enumerate(checkpoints):
        if (i + 1) % 20 == 0:
            console.print(f"📈 Progress: [cyan]{i+1}/{len(checkpoints)}[/cyan] | Time: [yellow]{as_of_ts.strftime('%H:%M')}[/yellow]")

        # Filter data to this point in time
        time_point_aware = as_of_ts.replace(second=0, microsecond=0)
        ...
        for symbol, timeframe_data in stock_multi_data.items():
            ...
            # Filter timeframe data to current time point
...
def run_live_mode_clean(stocks):
    """Live trading mode with enhanced indicators for buyers"""
    ...
    try:
        while (market_start <= datetime.now(IST) <= market_end):
            ...
            console.print(f"\n🔄 [bold yellow]LIVE SCAN {scan_count}[/bold yellow] | {current_time.strftime('%H:%M:%S')} IST")
            ...
            if scan_count % 10 == 0:
                total_signals = sum(performance_metrics.values())
                console.print(f"\n📊 [bold blue]Session Summary (Scan {scan_count})[/bold blue]")
                console.print(f"Total Signals: [cyan]{total_signals}[/cyan]")
        ...
        total_signals = sum(performance_metrics.values())
        console.print(f"\n📈 [bold green]LIVE SESSION COMPLETE[/bold green]")
...
def main():
    ...
    try:
        ...
        if args.test_oi:
            ...
            console.print("\n🎯 [bold green]Option Signal Scanner - Option Buyer Enhanced[/bold green]")
