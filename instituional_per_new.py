...
def run_backtest_day(day_str: str, stocks):
    ...
    for asof_ts in checkpoints:
        ...
        width = 120
        print("\n" + "="*width)
        hdr = f"| OPTION BUYER SCANNER | SNAPSHOT AT {asof_ts.strftime('%Y-%m-%d %H:%M')} IST"
        ...
        if not top_bullish:
            ...
            for r in top_bullish:
                action = "Consider Call" if 'Buy' in r['signal'] else "Consider Put"
                print(f"| {r['symbol']:<15} | {r['signal']:<18} | {r['score']:>7.2f} | {'NA':>10} | {r['trend']:<10} | {r.get('flow','Unknown'):<26} | {action:<19} |")
...
# ---------------- Main (with backtest modes) ----------------
def main():
    parser = argparse.ArgumentParser(description="Options buyer scanner with institutional flow and backtest modes")
