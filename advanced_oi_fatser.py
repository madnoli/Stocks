...
class TokenBucketLimiter:
    def __init__(self, rate_per_sec: float, bucket_size: int):
        ...
        self.last_refill = timemodule.time()
...
class EnhancedOptionBuyerScanner:
    def __init__(self, mode='live', backtest_date=None):
        ...
        self.td_hist_sessions = 5
        self.max_workers = 64
        self.tdhist_pool = []
        self.sess_limiters = []
        self.full_data_cache = None
        for i in range(self.td_hist_sessions):
            sess = TD_hist(TDUSERNAME, TDPASSWORD, log_level=logging.WARNING)
            self.tdhist_pool.append(sess)
        per_sess_rate = 10.0 / len(self.tdhist_pool)
        self.sess_limiters = [TokenBucketLimiter(rate_per_sec=per_sess_rate, bucket_size=10) for _ in self.tdhist_pool]
    ...
    def pick_session(self, symbol, tf):
        return (hash(symbol) ^ hash(str(tf))) % len(self.tdhist_pool)

    def fetch_one(self, symbol, tf, limiter, hist, start_time=None, end_time=None, duration=None, bar_size=None):
        td_symbol = symbol.replace('-I', '')
        try:
            limiter.acquire()
            if duration:
                rawdf = hist.get_historic_data(td_symbol, duration=duration, bar_size=bar_size)
            else:
                rawdf = hist.get_historic_data(td_symbol, start_time=start_time, end_time=end_time, bar_size=bar_size)
            df = self.normalize_live_data(rawdf, symbol)
            return symbol, tf, df
        except Exception as e:
            logger.error(f"Error fetching {symbol} {tf}: {e}")
            return symbol, tf, None

    def prefetch_data(self, symbols, timeframes, start_time=None, end_time=None, durations=None, bar_sizes=None, is_backtest=False):
        if is_backtest and self.full_data_cache:
            return self.full_data_cache
        stock_multi_data = defaultdict(dict)
        total_calls = len(symbols) * len(timeframes)
        with tqdm(total=total_calls, desc="Fetching data", ncols=100, leave=False) as api_bar:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = []
                for symbol in symbols:
                    for i, tf in enumerate(timeframes):
                        session_idx = self.pick_session(symbol, tf)
                        hist = self.tdhist_pool[session_idx]
                        limiter = self.sess_limiters[session_idx]
                        if durations:
                            dur = durations[i]
                            bs = bar_sizes[i]
                            f = executor.submit(self.fetch_one, symbol, tf, limiter, hist, duration=dur, bar_size=bs)
                        else:
                            f = executor.submit(self.fetch_one, symbol, tf, limiter, hist, start_time=start_time, end_time=end_time)
                        futures.append(f)
                for fut in as_completed(futures):
                    sym, tf, df = fut.result()
                    if df is not None and len(df) >= 20:
                        stock_multi_data[sym][tf] = df
                    api_bar.update(1)
        data = {s: d for s, d in stock_multi_data.items() if len(d) >= 3}
        if is_backtest:
    ...
    def normalize_live_data(self, df, symbol):
        try:
            ...
            if not isinstance(dfc.index, pd.DatetimeIndex):
                logger.error(f"Index not datetime for {symbol}")
                return None
    ...
    # NEW: Session filter for backtest intraday
    def filter_to_session(self, df):
        """
        Keep only bars between 09:15 and 15:30 IST, Mon–Fri.
        For intraday dataframes; returns df unchanged if empty/bad.
        """
        ...
        try:
            if pd.api.types.is_datetime64tz_dtype(df.index):
                ...
                df_ist = df.copy()
                df_ist.index = df_ist.index.tz_localize("Asia/Kolkata", nonexistent="shift_forward", ambiguous="NaT")

            df_ist = df_ist[(df_ist.index.weekday <= 4)]
            df_ist = df_ist[(df_ist.index.time >= self.market_start) & (df_ist.index.time <= self.market_end)]

            df_ist.index = df_ist.index.tz_convert(None)
            return df_ist
        except Exception:
            return df[(df.index.weekday <= 4) &
                      (df.index.time >= self.market_start) &
                      (df.index.time <= self.market_end)]
    ...
    def enhanced_scan_cycle(self):
        ...
        tf_map = {5: "5 min", 15: "15 min", 30: "30 min", 60: "60 min", "daily": "EOD"}
        days_map = {5: 10, 15: 10, 30: 20, 60: 60, "daily": 365}
        durations = [f"{days_map[tf]} D" if tf != "daily" else "365 D" for tf in timeframes]
        bar_sizes = [tf_map[tf] for tf in timeframes]
        if self.mode == 'backtest':
            if self.full_data_cache is None:
                start_time_fetch = self.backtest_date - timedelta(days=max(days_map.values()))
                end_time_fetch = self.backtest_date + timedelta(hours=15, minutes=30)
                self.full_data_cache = self.prefetch_data(target_stocks, timeframes, start_time=start_time_fetch, end_time=end_time_fetch, is_backtest=True)
            all_data = self.full_data_cache
            timeframes_data_per_stock = {}
            for symbol in target_stocks:
                if symbol in all_data:
                    timeframes_data = {}
                    for tf, df_full in all_data[symbol].items():
                        if df_full is not None and not df_full.empty and isinstance(df_full.index, pd.DatetimeIndex):
                            df_filtered = self.filter_to_session(df_full[df_full.index <= current_time])
                            if not df_filtered.empty and len(df_filtered) >= 20:
                                timeframes_data[tf] = df_filtered
                    if len(timeframes_data) >= 3:
                        timeframes_data_per_stock[symbol] = timeframes_data
        else:
            all_data = self.prefetch_data(target_stocks, timeframes, durations=durations, bar_sizes=bar_sizes)
            timeframes_data_per_stock = {}
            for symbol in target_stocks:
                if symbol in all_data:
                    timeframes_data = {}
                    for tf, df in all_data[symbol].items():
        ...
        def process_stock(symbol):
            try:
                timeframes_data = timeframes_data_per_stock.get(symbol, {})
                if len(timeframes_data) >= 3:
