...
class EnhancedOptionBuyerScanner:
    ...
    def fetch_live_data(self, symbol, timeframe):
        try:
            ...
            # NEW: enforce session hours in backtest for intraday frames
            if self.mode == 'backtest' and timeframe in (5, 15, 30, 60):
                normalized_df = self.filter_to_session(normalized_df)
                if normalized_df is None or len(normalized_df) < 20:
                    return None, False
