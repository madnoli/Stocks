...
ENHANCED_INDICATOR_WEIGHTS = {
    ...
    "VolumeSMA3x": 2.4,       # Volume 3x SMA filter for high activity
...
# =========================
# --- NEW OPTION-CHAIN METRIC WEIGHTS ---
# =========================
...
# Sector to stocks mapping (trimmed list; extend as needed)
SECTOR_STOCKS = {
...
def build_option_chain_scores(symbol):
    ...
    try:
        ...
        if not is_valid:
            logger.warning(f"Invalid option chain for {symbol}: {msg}")
            return {}
        ...
        try:
            ...
            logger.error(f"PCR calculation error for {symbol}: {e}")
        ...
        try:
            ...
            logger.error(f"OI momentum calculation error for {symbol}: {e}")
        ...
        try:
            ...
            logger.error(f"IV attractiveness calculation error for {symbol}: {e}")
        ...
        try:
            ...
            logger.error(f"Max pain calculation error for {symbol}: {e}")
        ...
        try:
            ...
            logger.error(f"Institutional flow calculation error for {symbol}: {e}")
        ...
        try:
            ...
            logger.error(f"Strike S/R calculation error for {symbol}: {e}")
            scores["Strike_SR_Score"] = 50.0

        logger.info(f"Option chain scores calculated for {symbol}: {len(scores)} metrics")
        return scores
        
    except Exception as e:
        logger.error(f"Option chain scoring failed for {symbol}: {e}")
        return {}
...
class EnhancedOptionBuyerIndicators:
    ...
    def calculate_all_indicators(df):
        ...
        try:
            ...
            # 21. Volume SMA 3x Filter (NEW INDICATOR)
            if len(df) >= 20:
                volume_sma_20 = vol.rolling(window=20).mean()
                volume_ratio = vol / volume_sma_20.replace(0, np.nan)
                # Score based on volume ratio: 3x or more gets 100, linearly scaled below
                volume_multiplier_score = np.clip(volume_ratio / 3.0 * 100, 0, 100)
                indicators["VolumeSMA3x"] = volume_multiplier_score
            else:
                indicators["VolumeSMA3x"] = pd.Series([50] * len(df), index=df.index)
...
def normalize_indicator_value(indicator_name, value):
    ...
    try:
        if indicator_name == "RSI":
            ...
            return 50
        elif indicator_name == "VolumeSMA3x":
            return max(0, min(100, value))
...
class EnhancedOptionBuyerScanner:
    ...
    def display_option_buyer_signals(self, signals, scan_time, gapdown_filtered, current_time):
        ...
        console.print(f"[blue]🎯 Enhanced for Option Buyers: Volume-OI Flow + Institutional Activity + Options Momentum + Option Chain[/]")
...
class PerformanceMonitor:
    def __init__(self):
        ...
        self.option_chain_times = []
        self.signal_counts = []
        
    def record_scan_time(self, time_taken):
        self.scan_times.append(time_taken)
        if len(self.scan_times) > 50:
            self.scan_times = self.scan_times[-50:]
    
    def record_api_time(self, time_taken):
        self.api_response_times.append(time_taken)
        if len(self.api_response_times) > 100:
            self.api_response_times = self.api_response_times[-100:]
    
    def record_option_chain_time(self, time_taken):
        self.option_chain_times.append(time_taken)
        if len(self.option_chain_times) > 100:
            self.option_chain_times = self.option_chain_times[-100:]
    
    def record_signal_count(self, count):
        self.signal_counts.append(count)
        if len(self.signal_counts) > 50:
            self.signal_counts = self.signal_counts[-50:]
    
    def get_stats(self):
        stats = {}
        if self.scan_times:
            stats['avg_scan_time'] = np.mean(self.scan_times)
            stats['max_scan_time'] = max(self.scan_times)
        if self.api_response_times:
            stats['avg_api_time'] = np.mean(self.api_response_times)
            stats['max_api_time'] = max(self.api_response_times)
        if self.option_chain_times:
            stats['avg_option_chain_time'] = np.mean(self.option_chain_times)
            stats['max_option_chain_time'] = max(self.option_chain_times)
        if self.signal_counts:
            stats['avg_signals'] = np.mean(self.signal_counts)
            stats['max_signals'] = max(self.signal_counts)
        return stats
...
class ConfigManager:
    ...
    def save_config(scanner):
        ...
        config = {
            'best_sectors': scanner.best_sectors,
            'worst_sectors': scanner.worst_sectors,
            'last_update': scanner.last_sectoral_update.isoformat() if scanner.last_sectoral_update else None,
            'indicator_weights': ENHANCED_INDICATOR_WEIGHTS,
            'option_chain_weights': OPTION_CHAIN_WEIGHTS,
    ...
    def load_config():
        ...
        try:
            ...
            print(f"{Colors.YELLOW}No saved configuration found{Colors.RESET}")
            return None
        except Exception as e:
...
def interactive_mode():
    ...
    while True:
        ...
        try:
            choice = input(f"\n{Colors.CYAN}Enter choice (0-9): {Colors.RESET}").strip()
            ...
            if choice == '0':
                ...
                scanner = EnhancedOptionBuyerScanner(mode='live')
                ConfigManager.save_config(scanner)
            elif choice == '9':
...
def quick_test_symbols():
    """Quick test of symbol cleaning and option chain availability"""
    print(f"{Colors.CYAN}QUICK SYMBOL TEST{Colors.RESET}")
    print("=" * 50)
    
    test_symbols = [
        "RELIANCE-I", "TCS-I", "HDFCBANK-I", "VEDL-I", "TATASTEEL-I",
        "ICICIBANK-I", "SBIN-I", "INFY-I", "HINDUNILVR-I", "ITC-I"
    ]
    
    for symbol in test_symbols:
        cleaned = clean_symbol_for_options(symbol)
        has_options = validate_symbol_for_options(symbol)
        chain = fetch_option_chain(symbol)
        
        status = f"{Colors.GREEN}✓{Colors.RESET}" if chain else f"{Colors.RED}✗{Colors.RESET}"
        strikes = len(chain.get('records', {}).get('data', [])) if chain else 0
        
        print(f"{symbol:12} -> {cleaned:10} | Options: {'Yes' if has_options else 'No':3} | Chain: {status} ({strikes} strikes)")

def benchmark_option_chain_speed():
    """Benchmark option chain API response times"""
    print(f"{Colors.CYAN}OPTION CHAIN API SPEED BENCHMARK{Colors.RESET}")
    print("=" * 50)
    
    test_symbols = ["RELIANCE-I", "TCS-I", "HDFCBANK-I", "VEDL-I", "TATASTEEL-I"]
    times = []
    ...
    for symbol in test_symbols:
        ...
        status = f"{Colors.GREEN}Success{Colors.RESET}" if chain else f"{Colors.RED}Failed{Colors.RESET}"
        print(f"{symbol:12}: {response_time:.2f}s - {status}")
    ...
    if times:
        ...
        print(f"\n{Colors.YELLOW}Benchmark Results:{Colors.RESET}")
...
def create_sample_config():
    ...
    sample_config = {
        ...
        "timeframe_weights": TIMEFRAME_WEIGHTS,
        "sectors": {
            "best": ["Technology", "Pharma", "Banking", "Finance"],
            "worst": ["PSU Bank", "Realty", "Energy", "Defence"]
        },
        "api_endpoints": {
            "indices": "http://localhost:3000/api/allIndices",
            "option_chain": "http://localhost:3000/api/equity/options/"
        }
    }
    ...
    try:
        ...
        print(f"{Colors.GREEN}Sample configuration created: sample_config.json{Colors.RESET}")
    except Exception as e:
        print(f"{Colors.RED}Error creating sample config: {e}{Colors.RESET}")

def show_help():
    """Display help information"""
    print(f"{Colors.CYAN}ENHANCED OPTION BUYER SCANNER - HELP{Colors.RESET}")
    print("=" * 60)
    
    print(f"\n{Colors.YELLOW}COMMAND LINE USAGE:{Colors.RESET}")
    print("python scanner.py                    # Interactive mode")
    print("python scanner.py --live             # Live scanning mode")
    print("python scanner.py --backtest 2025-10-01  # Backtest mode")
    print("python scanner.py --interactive      # Interactive mode")
    print("python scanner.py --test-option-chain --symbol VEDL-I")
    print("python scanner.py --test-stock --symbol VEDL-I")
    print("python scanner.py --validate         # Data validation")
    print("python scanner.py --report           # Sample report")
    print("python scanner.py --help             # This help")
    
    print(f"\n{Colors.YELLOW}FEATURES:{Colors.RESET}")
    print("• Multi-timeframe technical analysis (5m, 15m, 30m, 60m, Daily)")
    print("• Volume and Open Interest flow analysis")
    print("• Option chain integration (PCR, IV, Max Pain)")
    print("• Institutional activity detection")
    print("• Sector-based filtering and scoring")
    print("• Real-time market sentiment analysis")
    
    print(f"\n{Colors.YELLOW}OPTION CHAIN METRICS:{Colors.RESET}")
    print("• PCR (Put-Call Ratio) - Market sentiment")
    print("• OI Momentum - Institutional positioning")
    print("• IV Attractiveness - Premium opportunities")
    print("• Max Pain Distance - Price resistance levels")
    print("• Strike Support/Resistance - Key option levels")
    
    print(f"\n{Colors.YELLOW}REQUIREMENTS:{Colors.RESET}")
    print("• TrueData API credentials (TD_USERNAME, TD_PASSWORD)")
    print("• Local API server at localhost:3000")
    print("• Required packages: pandas, numpy, requests, rich, truedata")
...
if __name__ == "__main__":
    ...
    print(f"{Colors.CYAN}Version 2.0 - October 2025{Colors.RESET}")
    print(f"{Colors.CYAN}Features: Volume+OI Flow, Institutional Analysis, Option Chain PCR/IV/MaxPain{Colors.RESET}")
    print("=" * 80)
