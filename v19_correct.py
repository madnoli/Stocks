# Enhanced Scanner Top-20 v24-Compatible (TrueData, 5/15/30/60/1D, unified indicators, multi-session 10 rps, tz-aware, single progress bar)
import os
import logging
import warnings
...
# Timeframe and indicator weights (same spirit as v24 TEST)
TIMEFRAME_WEIGHTS = {5: 1.0, 15: 1.2, 30: 1.4, 60: 1.6, 1440: 2.0}
INDICATOR_WEIGHTS = {
    "RSI": 1.3, "MACD": 1.6, "Stochastic": 1.0, "MA": 1.8, "ADX": 1.5,
    "Bollinger": 1.4, "ROC": 1.2, "OBV": 1.6, "CCI": 1.1, "WWL": 1.0,
    "EMA": 1.7, "VWAP": 1.5, "ATR": 1.4, "VolumeSurge": 2.0, "Momentum": 1.9
}
...
# ---------------- Scoring (exact v24 TEST approach) ----------------
def get_indicator_scores(df):
    ...
    # New indicators (same as v24 TEST)
...
def live_market_scanner_top20():
    ...
    while True:
        ...
        # Rank and select top 20 bullish/bearish (v24 logic, just top counts changed)
