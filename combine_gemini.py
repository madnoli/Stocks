...
# ---- Credentials ----
TDUSERNAME = os.getenv("TDUSERNAME", "tdwsp751")
TDPASSWORD = os.getenv("TDPASSWORD", "raj@751")
...
def normalize_truedata_df(df):
    try:
        ...
        # --- FIX #1: Localize the index to make it timezone-aware ---
        dfc = dfc.tz_localize('Asia/Kolkata', ambiguous='infer')
...
# ---- Main Scanner Logic ----
def main_interval_scanner(run_date_str=None, interval_minutes=5, max_workers=10, continuous=True):
    ...
    # --- FIX #2: Entire process_at function is replaced for clarity and correctness ---
    def process_at(time_point_aware):
        ...
        for symbol, timeframe_data in local_cache_copy.items():
            # This comparison will now work correctly as both are tz-aware
        ...
        # Render table
        print("\n" + "="*120)
        print(f"| SIGNALS AT {time_point_aware.strftime('%H:%M')} IST | Scanned: {len(local_cache_copy)}/{len(ALL_NSE_STOCKS)}".center(118) + "|")
        print("="*120)
        if not signals:
    ...
    while True:
        ...
        if now.time() > session_ends and continuous:
            logger.info("Market session ended. Shutting down.")
            break
        
        # In continuous mode, queue 5-min data for refresh every 60 seconds
        if continuous and (time_module.time() - last_refresh_time > 60):
            logger.info("Queueing 5-min data refresh for all stocks.")
            for stock in ALL_NSE_STOCKS:
                FETCH_QUEUE.put((stock, [5], True)) # True = is_refresh
            last_refresh_time = time_module.time()

        next_boundary = (now + timedelta(minutes=5)).replace(minute=(now.minute // 5) * 5, second=1, microsecond=0)
        if next_boundary <= now:
            next_boundary += timedelta(minutes=5)

        if now.time() < session_starts and continuous:
            sleep_duration = (ist.localize(analysis_date.replace(hour=session_starts.hour, minute=session_starts.minute)) - now).total_seconds()
            logger.info(f"Before session. Sleeping for {sleep_duration:.0f}s until 09:15.")
            if sleep_duration > 0: time_module.sleep(min(sleep_duration, 60))
            continue
        ...
        # --- FIX #3: Pass the timezone-aware datetime object directly ---
        process_at(next_boundary)
        
        if not continuous:
            logger.info("Non-continuous mode: scan finished.")
            break
...
if __name__ == "__main__":
    main_interval_scanner(
        run_date_str=None,      # None for today, or "YYYY-MM-DD" for backtesting
        interval_minutes=5,
        max_workers=10,         # Controls concurrency of API calls (max 10)
        continuous=True         # Set to False to run once on historical data
    )