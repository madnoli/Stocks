...
# ---------- Feature toggles / env ----------
# Keep options confirmation OFF until chain fetch is implemented to avoid dropping candidates
OPTIONS_CONFIRM = os.getenv("OPTIONS_CONFIRM", "0") == "1"  # enable after implementing fetch_option_chain [web:7]
# Disable index alignment influence for first run; re-enable after verifying symbols via TrueData symbol list docs
INDEX_ALIGN_DISABLE = os.getenv("INDEX_ALIGN_DISABLE", "1") == "1"  # default 1 for initial testing [web:60]
...
TIMEFRAME_WEIGHTS = {1: 1.8, 5: 1.6, 15: 1.3, 30: 1.1, 60: 0.8, 1440: 0.5}  # 1–5m emphasized for entries [web:52]
INDICATOR_WEIGHTS = {
...
}  # VWAP/EMA cluster + volume surge priority for option buys [web:72]

BAR_SIZE_MAP = {1: "1 min", 5: "5 min", 15: "15 min", 30: "30 min", 60: "60 min", 1440: "1 day"}  # TFs [web:52]
DURATION_MAP = {1: "5 D", 5: "30 D", 15: "30 D", 30: "60 D", 60: "120 D", 1440: "365 D"}  # windows [web:52]
...
# Relaxed thresholds to surface names midday
MIN_ATR_PCT_5M = float(os.getenv("MIN_ATR_PCT_5M", "0.007"))  # 0.7% [web:52]
MIN_VOL_Z_PACE = float(os.getenv("MIN_VOL_Z_PACE", "1.05"))   # slightly above average [web:52]
MAX_VWAP_ATR_DIST = float(os.getenv("MAX_VWAP_ATR_DIST", "1.8"))  # farther trends [web:52]
MIN_ADX = float(os.getenv("MIN_ADX", "18.0"))  # earlier trend [web:52]
RSI_BUY_MIN = float(os.getenv("RSI_BUY_MIN", "50.0"))
RSI_BUY_MAX = float(os.getenv("RSI_BUY_MAX", "78.0"))
VOL_Z_ENTRY = float(os.getenv("VOL_Z_ENTRY", "1.2"))  # moderate surges [web:52]

# Diagnostics toggles
DEBUG_PREFILTER = os.getenv("DEBUG_PREFILTER", "1") == "1"
DEBUG_ENTRY = os.getenv("DEBUG_ENTRY", "1") == "1"

# ---------------- TrueData sessions ----------------
def authenticate_session():
    return TD_hist(TDUSERNAME, TDPASSWORD, log_level=logging.CRITICAL)  # Market Data API auth [web:60]
...
def build_sessions():
    ...
    per_sess_rate = 10.0 / len(pool)  # global cap split [web:69]
...
# ---------------- Index symbols ----------------
# If these don’t load on your account, disable index bias (INDEX_ALIGN_DISABLE=1) and later replace with exact names from TrueData symbol list [web:60][web:81]
...
def rps_monitor(stop_event, bar):
    ...
    while not stop_event.is_set():
        time.sleep(2.0)  # reduce overhead of frequent postfix updates
        with api_calls_lock:
            curr = api_calls_done
        rps = curr - prev
        prev = curr
