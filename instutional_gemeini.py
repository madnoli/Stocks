...
def next_5min_boundary_ist(now_ist: datetime) -> datetime:
    # Move to next HH:MM where MM % 5 == 0 and seconds=0
...
ENHANCED_INDICATOR_WEIGHTS = {
    "VolumeSurge": 2.2, "Momentum": 2.1, "ATR": 1.8, "ROC": 1.5, # Emphasize volatility/momentum
...
# ---------- New Volatility Helper ----------
def get_volatility_metric(df, period=14):
    """Calculates ATR as a percentage of the last close price."""
...
def run_live_5min():
    ...
    while True:
        ...
        logger.info("Data refresh complete. Filtering for liquid, option-friendly stocks...")
        eligible_stocks = {}
        ...
        logger.info(f"Found {len(eligible_stocks)} eligible stocks after filtering. Analyzing signals...")
        ...
        for symbol, timeframe_data in eligible_stocks.items():
            ...
            if 'Strong' in signal:
                ...
                # Use 60-min or 30-min timeframe for a stable volatility reading
        ...
        if not top_bullish:
            ...
            print(f"| {'Stock':<15} | {'Signal':<18} | {'Score':>7} | {'ATR %':>7} | {'Change':>10} | {'Trend':<10} | {'Flow':<26} | {'Action':<19} |")
            print("-"*width)
            for result in top_bullish:
                ...
                if isinstance(change_val, (int, float, np.floating)):
                    ...
                    change_str = "NA"
                action = "Consider Call" if 'Buy' in result['signal'] else "Consider Put"
                print(f"| {result['symbol']:<15} | {result['signal']:<18} | {result['score']:>7.2f} | {result['volatility_pct']:>6.2f}% | {change_str:>10} | {result['trend']:<10} | {result.get('flow','Unknown'):<26} | {action:<19} |")
        ...
        with open(output_filename, "a", encoding="utf-8") as f:
            f.write(f"\n===== Scan Time: {now_ist.strftime('%Y-%m-%d %H:%M')} =====\n\n")
            f.write("Top 20 Bullish (Momentum Breakouts)\n")
            f.write(f"{'Stock':<15} | {'Signal':<18} | {'Score':>7} | {'ATR %':>7} | {'Change':>10} | {'Trend':<10} | {'Flow':<26} | {'Action':<19}\n")
            f.write("-" * 130 + "\n")
            if not top_bullish: f.write("No strong bullish signals found.\n")
            for r in top_bullish:
                change_val = r['change']
                change_str = "NA" if not isinstance(change_val, (int, float, np.floating)) else f"{'+' if change_val > 0 else ''}{change_val:>.2f}"
                action = "Consider Long" if 'Buy' in r['signal'] else "Consider Short"
                f.write(f"{r['symbol']:<15} | {r['signal']:<18} | {r['score']:>7.2f} | {r['volatility_pct']:>6.2f}% | {change_str:>10} | {r['trend']:<10} | {r.get('flow','Unknown'):<26} | {action:<19}\n")
            
            f.write("\nTop 20 Bearish (Momentum Breakdowns)\n")
            f.write(f"{'Stock':<15} | {'Signal':<18} | {'Score':>7} | {'ATR %':>7} | {'Change':>10} | {'Trend':<10} | {'Flow':<26} | {'Action':<19}\n")
            f.write("-" * 130 + "\n")
            if not top_bearish: f.write("No strong bearish signals found.\n")
