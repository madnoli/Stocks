...
def calculate_option_quality_score(df, symbol):
    ...
    try:
        ...
        # Add historical volatility for better option premium assessment (20% additional for option buyers)
        hv = historical_volatility(df)
        if pd.notna(hv):
            if 20 < hv < 50:  # Optimal range for option buyers (reasonable premium expansion potential)
                quality_score += 20
            elif 10 < hv < 70:  # Broader range with partial credit
                quality_score += 10
            else:
                quality_score += 5  # Minimal credit for extreme vols
...
def historical_volatility(df, period=20):
    """Calculate historical volatility (annualized) for better option premium assessment"""
    if len(df) < period + 1:
        return np.nan
    returns = np.log(df['Close'] / df['Close'].shift(1))
    std = returns.rolling(period).std() * np.sqrt(252)  # Annualized volatility in %
    return std.iloc[-1]
