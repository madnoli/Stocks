...
class EnhancedOptionBuyerScanner:
        ...
        def process_stock(symbol):
            try:
                ...
                if len(timeframes_data) >= 3:
                    ...
                    volume_multiplier = 0
                    if small_df is not None and len(small_df) >= 20 and 'Volume' in small_df.columns:
                        sma_vol_20 = small_df['Volume'].rolling(window=20).mean().iloc[-1]
                        current_vol = small_df['Volume'].iloc[-1]

                        if sma_vol_20 > 0:
                            volume_multiplier = current_vol / sma_vol_20
                            # Filter out if volume is not at least 3x the SMA
                            if volume_multiplier < 3.0:
                                return None, False
                        else:
                            return None, False # Cannot calculate, filter out
                    else:
                        return None, False # Not enough data, filter out
                    # --- END OF NEW VOLUME CONDITION ---
                    ...
                    if abs(score - 50) >= 12:
                        ...
                        result = {
                            ...
                            "oi_change": oi_change,
                            "volumeX": volume_multiplier,  # Add volume multiplier to result
                        }
    ...
    def display_option_buyer_signals(self, signals, scan_time, gapdown_filtered, current_time):
        ...
        console.print(f"{'-'*150}")
        console.print(f"Analysis: [yellow]5m[/] [yellow]15m[/] [yellow]30m[/] [cyan]60m[/] [magenta]Daily[/] | [green]Volume+OI Flow[/] | [bold red]Vol > 3x SMA[/]")
        ...
        if not signals:
            console.print(f"[yellow]No significant option buying opportunities found in this cycle (Vol > 3x SMA).[/]")
        else:
            ...
            call_table.add_column("Vol x SMA", justify="right", style="cyan") # New Column
            ...
            for s in call_signals[:15]:
                ...
                # New VolumeX display logic
                vol_x = s.get('volumeX', 0.0)
                vol_x_str = f"{vol_x:.1f}x"
                vol_x_style = "bold yellow" if vol_x >= 5.0 else "cyan"
                ...
                call_table.add_row(
                    ...
                    f"{s['score']:.1f}",
                    delta_display,
                    f"[{vol_x_style}]{vol_x_str}[/]", # Added column data
                    strength,
                    vol_str,
                    vol_change_str,
                    oi_str,
            ...
            put_table.add_column("Vol x SMA", justify="right", style="cyan") # New Column
