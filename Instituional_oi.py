...
# === ENHANCEMENTS FOR OPTION BUYERS ===
# Replace with your real option data API call
def get_option_chain(symbol):
    # Dummy example data structure for demo; replace with your API call
    return [
        {"strikePrice": 500, "callOIChange": 800, "callIVChange": 1.2, "callDelta": 0.45},
        {"strikePrice": 510, "callOIChange": 900, "callIVChange": 1.1, "callDelta": 0.62},
    ]

def analyze_option_chain(symbol, underlying_price, atr, strike_window=1.5):
    try:
        chain = get_option_chain(symbol)
        for s in chain:
            strike = s["strikePrice"]
            if abs(strike - underlying_price) <= strike_window * atr:
                if s["callOIChange"] > 0 and s["callIVChange"] > 0 and s["callDelta"] > 0.2:
                    return True
        return False
    except Exception as e:
        logger.error(f"Option chain API failed for {symbol}: {e}")
        return False

def valid_breakout_range(df, atr_period=14, threshold=1.2):
    close = df["Close"].iloc[-1]
    atr_val = atr(df, period=atr_period).iloc[-1]
    resistance = df["High"].rolling(window=20).max().iloc[-2]
    distance = resistance - close
    return distance > threshold * atr_val

def get_time_weight(now_ist):
    hour = now_ist.hour
    if 9 <= hour < 11:
        return 1.3
    elif 13 <= hour < 15:
...
def run_live_5min():
    ...
    while True:
        ...
        for symbol, timeframe_data in stock_multi_data.items():
            ...
            # --- Enhanced filters for option buyers ---
            df_5 = filtered_timeframes.get(5)
            if df_5 is not None and len(df_5) > 20:
                underlying_price = df_5["Close"].iloc[-1]
                atr_val = atr(df_5, period=14).iloc[-1]
                if not valid_breakout_range(df_5, atr_period=14, threshold=1.2):
                    continue
                if not analyze_option_chain(symbol, underlying_price, atr_val, strike_window=1.5):
                    continue

            time_weight = get_time_weight(time_point_aware)
        ...
        if not top_bullish:
            ...
            for result in top_bullish:
                ...
                print(f"| {result['symbol']:<15} | {signal_text:<18} | {result['score']:>7.2f} | {change_str:>{padding}} | {result['trend']:<10} | {result.get('flow','Unknown'):<26} | {action:<19} |")
