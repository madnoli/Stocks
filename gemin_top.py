# ==============================================================================
# ENHANCED OPTION BUYER SCANNER v3.4 - UPDATED FOR QUALITY SIGNALS & SUPPORT/RESISTANCE BREAKOUTS
# Improvements: 
# - Prioritized quality over quantity: Increased SCORE_THRESHOLD_MIN to 20.0 and adjusted SIGNAL_THRESHOLDS
# - Added Support/Resistance Breakout detection in Trend group for better signal accuracy
# - Uses scipy.signal.find_peaks for detecting maxima/minima and breakout conditions
# - Added Supertrend indicator for trend confirmation
# - Added Pivot Point breakout detection
# - Added volume confirmation to breakout detection
# ==============================================================================
import os
import logging
import warnings
...
import scipy.signal as sp  # Added for breakout detection
...
class Config:
    ...
    INDICATOR_WEIGHTS = {
        ...
        "VolatilityExpansion": 2.5, "Bollinger": 1.3, "BollingerSqueeze": 1.5,
        "Breakout": 2.0,  # Added for Support/Resistance Breakout
        "PivotBreakout": 1.8,  # Added for Pivot Point Breakout
        "Supertrend": 1.8,  # Added for Supertrend
        "OptionBuyerMomentum": 2.8, "OIChange": 2.5, "VolumeOISync": 2.2,
    }
    # --- Scoring & Signal Thresholds (Updated for Quality Signals: Higher thresholds for fewer, stronger signals) ---
    SCORE_THRESHOLD_MIN = 20.0  # Increased from 10.0 for quality over quantity
    SIGNAL_THRESHOLDS = {
        'Very Strong Buy': 60.0, 'Strong Buy': 40.0, 'Buy Signal': 25.0,  # Adjusted upwards
        'Very Strong Sell': -60.0, 'Strong Sell': -40.0, 'Sell Signal': -25.0,
    }
...
BAR_SIZE_MAP = {5: "5min", 15: "15min", 30: "30min", 60: "60min"}  # Removed 1440/EOD
DURATION_MAP = {5: "30D", 15: "30D", 30: "60D", 60: "120D"}  # Removed 1440
TIMEFRAME_WEIGHTS = {15: 3.0, 5: 2.5, 30: 2.0, 60: 1.5}  # Removed 1440
# Silence noisy loggers
for noisy in ("truedata", "truedata.history", "truedata_ws", "websocket", "urllib3"):
    logging.getLogger(noisy).setLevel(logging.CRITICAL)
# FIXED: Enhanced state management for proper Volume/OI tracking
...
def create_great_table_fixed(data, title, new_stocks=None, show_time=None):
    """FIXED: Create beautiful tables using great-tables library with correct row selection."""
    if not data or not GREAT_TABLES_AVAILABLE:
        # Fallback to rich table
    ...
    try:
        ...
        # FIXED: Create great table with proper row selection using lambda
        gt_table = (
            GT(df)
            .tab_header(
                title=title,
            ...
            .tab_spanner(
                ...
                columns=["Stock", "Signal", "Score"]
            )
            .tab_spanner(
                label="Technical Indicators",
                columns=["Trend", "Momentum", "Volume", "OI"]
            )
            .tab_spanner(
                label="Current Data",
                columns=["Curr_Vol", "Curr_OI", "Vol_Change", "OI_Change"]
            )
            .tab_spanner(
                label="Analysis",
                columns=["Flow", "Action"]
            ...
            .fmt_number(
                columns=["Score", "Trend", "Momentum", "Volume", "OI"],
                decimals=2
            )
            .fmt_percent(
                columns=["Vol_Change", "OI_Change"],
                decimals=1
            )
            .data_color(
                columns=["Score"],
            ...
            .data_color(
                columns=["Vol_Change"],
                palette=["red", "white", "lightgreen"],
                domain=[-50, 50]
            )
            .data_color(
                columns=["OI_Change"],
                palette=["red", "white", "lightblue"],
                domain=[-50, 50]
            )
            # FIXED: Use lambda function for row selection
            .tab_style(
                style=style.fill(color="yellow"),
                locations=loc.body(rows=lambda df: df['Is_New'] == 1)
            )
            .cols_hide(columns=["Is_New"]) # Hide helper column
            .tab_options(
                table_font_size="12px",
                heading_background_color="#2E86AB",
                heading_text_color="white",
                stub_background_color="#F28E2C"
            )
        )
...
def create_rich_enhanced_table(data, title, new_stocks=None, show_time=None):
    """Enhanced table with Rich library including new Volume/OI columns."""
    if not data:
        if RICH_AVAILABLE:
        ...
        return
    if RICH_AVAILABLE:
        # Rich table with enhanced columns
        ...
        for item in data:
            ...
            # Style stock symbol
            if is_new:
                stock_style = "[bold bright_magenta]" + symbol + " ✨[/bold bright_magenta]"
            else:
                stock_style = symbol
            # Format volume change with colors
        ...
        # Fallback to enhanced ASCII table
...
def create_enhanced_ascii_table(data, title, new_stocks=None, show_time=None):
    """Enhanced ASCII table with new columns for systems without Rich."""
    if not data:
        print_colored(f"\n{title}", Colors.HEADER)
        print_colored("No stocks found in this category.", Colors.YELLOW)
        return
    # Header with time if provided
    if show_time:
...
def create_compact_backtest_table(data, title, new_stocks=None, show_time=None):
    ...
    # Use great-tables if available, otherwise fall back to rich
...
def bollinger_bandwidth(df, period=20, std_dev=2):
...
def slope(series, lookback=10):
    ...
    try:
        ...
        return 0.0
# ========== NEW: SUPPORT/RESISTANCE BREAKOUT DETECTION ==========
def detect_breakout(df, pivot_points=3, band_perc=0.005):
    """Detect bullish/bearish breakouts based on clustered support/resistance levels with volume confirmation."""
    if len(df) < 50:
        return 0.0
    ...
    # Find maxima (resistance candidates)
    maxima, _ = sp.find_peaks(df['High'], distance=5)
    if len(maxima) >= pivot_points:
        latest_maxima = df['High'].iloc[maxima[-pivot_points:]]
        mean_max = latest_maxima.mean()
        band_width = (latest_maxima.max() - latest_maxima.min()) / mean_max
        if band_width < band_perc and df['Close'].iloc[-1] > mean_max * (1 + band_perc / 2):
            return 2.0 if volume_confirmed else 0.0  # Bullish breakout only if volume confirmed
    
    # Find minima (support candidates)
    minima, _ = sp.find_peaks(-df['Low'], distance=5)
    if len(minima) >= pivot_points:
        latest_minima = df['Low'].iloc[minima[-pivot_points:]]
        mean_min = latest_minima.mean()
        band_width = (latest_minima.max() - latest_minima.min()) / mean_min
        if band_width < band_perc and df['Close'].iloc[-1] < mean_min * (1 - band_perc / 2):
            return -2.0 if volume_confirmed else 0.0  # Bearish breakout only if volume confirmed
    
    return 0.0

def calculate_pivot_points(df):
    """Calculate classic pivot points and detect breakouts."""
    if len(df) < 3:
        return 0.0
    ...
    if current_close > r1:
        return 2.0  # Bullish pivot breakout
    elif current_close < s1:
        return -2.0  # Bearish pivot breakout
    
    return 0.0
def calculate_supertrend_score(df, period=10, multiplier=3):
    """Calculate Supertrend and return score."""
    if len(df) < period:
        return 0.0
    ...
    direction = pd.Series(1, index=df.index)  # 1 for up, -1 for down
    ...
    for i in range(1, len(df)):
        ...
        if direction.iloc[i] == 1:
            ...
            if supertrend.iloc[i] < supertrend.iloc[i-1] or df['Close'].iloc[i-1] < supertrend.iloc[i-1]:
    ...
    if df['Close'].iloc[-1] > supertrend.iloc[-1]:
        return 2.0  # Bullish
    elif df['Close'].iloc[-1] < supertrend.iloc[-1]:
        return -2.0  # Bearish
    return 0.0
# ========== SCORING ENGINE ==========
...
def calculate_indicator_scores(df):
    ...
    try:
        ...
        # NEW: Support/Resistance Breakout in Trend Group
        scores['Breakout'] = detect_breakout(df)
        
        # NEW: Pivot Point Breakout
        scores['PivotBreakout'] = calculate_pivot_points(df)
        
        # NEW: Supertrend
        scores['Supertrend'] = calculate_supertrend_score(df)
        ...
        if not bb_upper.empty and not bb_lower.empty and len(bb_upper) > 0 and len(bb_lower) > 0:
            if df['Close'].iloc[-1] > bb_upper.iloc[-1]:
                ...
                scores['Bollinger'] = -2.0
        bb_width = bollinger_bandwidth(df)
        if not bb_width.empty and len(bb_width) > 50:
            avg_width = bb_width.rolling(50).mean().iloc[-1]
            current_width = bb_width.iloc[-1]
            if current_width < avg_width * 0.5:  # Squeeze detected
                _, pdi, ndi = calculate_adx(df)
                if not pdi.empty and pdi.iloc[-1] > ndi.iloc[-1]:
                    scores['BollingerSqueeze'] = 2.0  # Bullish squeeze
                else:
                    scores['BollingerSqueeze'] = -2.0  # Bearish squeeze
        # --- OI Group ---
        ...
        for group, weight in Config.GROUP_WEIGHTS.items():
            ...
            for indicator, ind_weight in Config.INDICATOR_WEIGHTS.items():
                if indicator in indicator_scores:
                    belongs_to_group = (
                        (group == 'Trend' and any(term in indicator for term in ['MA', 'ADX', 'VWAP', 'EMA', 'MACD', 'Breakout', 'PivotBreakout', 'Supertrend'])) or
                        ...
                        (group == 'Volatility' and any(term in indicator for term in ['Volatility', 'Bollinger', 'BollingerSqueeze'])) or
...
# ========== FIXED 5-MINUTE VOLUME/OI TRACKING FUNCTIONS ==========
def calculate_5min_volume_oi_changes(df, symbol, scan_time):
    """FIXED: Calculate volume/OI changes from 5-minute timeframe data."""
    try:
...
def extract_5min_volume_oi_data(df, symbol, time_point=None, is_live=False):
    """FIXED: Extract volume/OI data with proper 5-minute candle-to-candle comparison."""
    try:
...
def fetch_one_timeaware(symbol_orig, timeframe_minutes, limiter, hist, up_to_time):
    td_symbol = symbol_orig.replace('-EQ', '').replace('-I', '')
    ...
    try:
        ...
        if df_raw is None or len(df_raw) == 0:
            logger.warning(f"No data returned for {symbol_orig} {timeframe_minutes}min")
            return symbol_orig, timeframe_minutes, None
        df = normalize_hist_df(pd.DataFrame(df_raw), td_symbol)
...
def get_market_regime(index_symbol="NIFTY 50"):
    # Use intraday 60-min regime to avoid daily dependency
    try:
        si = pick_session(index_symbol, 60)
        df_raw = tdhist_pool[si].get_historic_data(index_symbol, duration="120D", bar_size="60min")
        df = normalize_hist_df(pd.DataFrame(df_raw), index_symbol)
...
# ========== FIXED MAIN SCANNER LOGIC ==========
def run_scan_at_time_5min_fixed(time_point_aware, stocks, market_regime, is_live=False):
    if is_live:
        stock_multi_data = prefetch_all(stocks, max_workers=Config.MAX_WORKERS)
    else:
        stock_multi_data = prefetch_all_timeaware(stocks, time_point_aware, max_workers=Config.MAX_WORKERS)
    ...
    for symbol, timeframe_data in stock_multi_data.items():
        ...
        for tf, df in timeframe_data.items():
            if df is not None and not df.empty:
                df_slice = df[df.index <= time_point_aware] if not is_live else df
        ...
        if abs(score) >= Config.SCORE_THRESHOLD_MIN:
            ...
            if tf_5min is not None and len(tf_5min) >= 2:
                oi_vol_data = extract_5min_volume_oi_data(tf_5min, clean_symbol, time_point_aware, is_live=is_live)
            else:
                main_tf_data = filtered_timeframes.get(15, filtered_timeframes.get(30, filtered_timeframes.get(60, None)))
            ...
            if 'Strong' in signal:
                action = f"Strong {'Call' if score > 0 else 'Put'} Buy"
...
def export_backtest_to_excel(results, filename):
    try:
        ...
        for row_idx, row in enumerate(summary_data, 1):
            ws_summary.cell(row=row_idx, column=1, value=row[0])
            ws_summary.cell(row=row_idx, column=2, value=row[1])
...
def generate_performance_report(results, backtest_date):
    ...
    max_val = max(hourly_signals.values()) if hourly_signals else 1
    for hour in sorted(hourly_signals.keys()):
        bar_length = min(50, int(hourly_signals[hour] * 50 / max_val))
        bar = "█" * bar_length
    ...
    pct = (strong_signals / total_signals * 100) if total_signals else 0
    print(f" Strong Signals: {strong_signals}/{total_signals} ({pct:.1f}%)")
...
def main_final_fixed():
    parser = argparse.ArgumentParser(description="FINAL FIXED Enhanced Options Buyer Scanner v3.4")
    ...
    if args.backtest:
        ...
        print_colored("\n🚀 STARTING FINAL FIXED LIVE SCANNER v3.4", Colors.GREEN)
        ...
        while True:
            ...
            print_colored(f"\n[{now_ist.strftime('%H:%M:%S')}] FINAL FIXED SCANNER v3.4 - Scan #{scan_count}", Colors.HEADER)
            market_regime = get_market_regime(Config.BENCHMARK_INDEX)
...
if __name__ == "__main__":
    try:
        print_colored("\n🎯 FINAL FIXED ENHANCED OPTION BUYER SCANNER v3.4", Colors.HEADER)
        print_colored("✅ Updated: Quality Signals + Support/Resistance Breakouts + Supertrend", Colors.GREEN)
        if GREAT_TABLES_AVAILABLE:
        ...
        print_colored("\n🔧 UPDATES APPLIED:", Colors.CYAN)
        print(" ✅ Quality Focus: Higher thresholds for fewer, stronger signals")
        print(" ✅ Breakouts: Added support/resistance breakout detection in Trend group")
        print(" ✅ Supertrend: Added Supertrend indicator for trend confirmation")
        print(" ✅ Pivot Points: Added pivot point breakout detection")
        print(" ✅ Fixed: Great Tables row selection using lambda functions")
        ...
        print(" 🎯 Quality signals with breakout and supertrend confirmation")
        ...
        print_colored("\n🎯 Starting Updated Scanner...", Colors.GREEN)
        main_final_fixed()
        ...
        print(" pip install pandas numpy tqdm rich colorama great-tables scipy")
        ...
        print_colored("\n🎯 Updated Enhanced Option Buyer Scanner v3.4 shutdown complete!", Colors.HEADER)
        print_colored("📊 Thank you for using the Updated Scanner with Quality Signals & Breakouts!", Colors.GREEN)
        if 'scan_count' in globals() and scan_count > 0:
            uptime_info = f"Completed {scan_count} scans successfully"
            print_colored(f"⏱️ Session Summary: {uptime_info}", Colors.CYAN)
# ========== END OF COMPLETE UPDATED SCANNER CODE ==========
