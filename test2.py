...
# Features: Automatic top 3 best/worst sector detection every 5 minutes with comprehensive debugging
import pandas as pd
import numpy as np
...
TIMEFRAME_WEIGHTS = {5: 1.0, 15: 1.5, 30: 2.0, 60: 2.5, 1440: 3.5}
INDICATOR_WEIGHTS = {
...
class Enhanced5MinLiveScanner:
    def __init__(self):
        ...
        ## MODIFICATION: Changed to store top 3 best and worst sectors
        self.best_sectors = ["Technology", "Pharma", "Banking"]
        self.worst_sectors = ["Auto", "Metal", "Consumer"]
    ...
    def show_sector_status(self):
        ...
        ## MODIFICATION: Display lists of 3 sectors
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if sectoral_performance:
                    ...
                    ## MODIFICATION: Update top 3 best and worst sectors
                    if len(sectoral_performance) >= 6:
                        self.best_sectors = [sectoral_performance[0]['sector'], sectoral_performance[1]['sector'], sectoral_performance[2]['sector']]
                        self.worst_sectors = [sectoral_performance[-1]['sector'], sectoral_performance[-2]['sector'], sectoral_performance[-3]['sector']]
    ...
    def display_sectoral_update(self, sectoral_performance, old_best, old_worst):
        ...
        ## MODIFICATION: Display updated top 3 sectors
        print(f"🏆 Top 3 Best: {Colors.GREEN}{Colors.BOLD}{', '.join(self.best_sectors)}{Colors.RESET}")
        print(f"📉 Top 3 Worst: {Colors.RED}{Colors.BOLD}{', '.join(self.worst_sectors)}{Colors.RESET}")
    ...
    def fetch_live_data(self, symbol, timeframe):
        ...
        try:
            tf_map = {5: '5 min', 15: '15 min', 30: '30 min', 60: '1 H', 1440: '1 D'}
    ...
    def calculate_multi_indicator_signals(self, symbol, timeframes_data):
        ...
        try:
            ...
            ## MODIFICATION: Tiered sector boost for top 3 best/worst
            sector_boost = 0
            if sector in self.best_sectors:
                if sector == self.best_sectors[0]:
                    sector_boost = 15
                elif sector == self.best_sectors[1]:
                    sector_boost = 10
                elif sector == self.best_sectors[2]:
                    sector_boost = 5
            elif sector in self.worst_sectors:
                if sector == self.worst_sectors[0]:
                    sector_boost = -15
                elif sector == self.worst_sectors[1]:
                    sector_boost = -10
                elif sector == self.worst_sectors[2]:
                    sector_boost = -5
    ...
    def enhanced_5min_scan_cycle(self):
        ...
        try:
            ## MODIFICATION: Get target stocks from the top 3 best and top 3 worst sectors
            ...
            if len(self.best_sectors) > 2 and self.best_sectors[2] in SECTOR_STOCKS:
                target_stocks_set.update(SECTOR_STOCKS[self.best_sectors[2]][:6])  # 6 from 3rd best
            ...
            if len(self.worst_sectors) > 2 and self.worst_sectors[2] in SECTOR_STOCKS:
                target_stocks_set.update(SECTOR_STOCKS[self.worst_sectors[2]][:6])  # 6 from 3rd worst
            ...
            print(f"🎯 Scanning {len(target_stocks)} stocks from top 3 best/worst sectors.")
    ...
    def display_enhanced_signals(self, signals, scan_time):
        ...
        ## MODIFICATION: Display top 3 best/worst sectors
        if self.last_sectoral_update:
            ...
            print(f"{Colors.MAGENTA}📊 Sectoral Update:{Colors.RESET} {Colors.YELLOW}{self.last_sectoral_update.strftime('%H:%M:%S')}{Colors.RESET} | "
                  f"🏆 Top 3 Best: {Colors.GREEN}{Colors.BOLD}{best_str}{Colors.RESET} | "
                  f"📉 Top 3 Worst: {Colors.RED}{Colors.BOLD}{worst_str}{Colors.RESET}")
        ...
        if not signals:
            ...
            for s in signals[:20]:
                ## MODIFICATION: Enhanced sector color coding and icons for top 3
                sector_color, sector_name = Colors.YELLOW, s['sector']
                if s['sector'] in self.best_sectors:
                    rank = self.best_sectors.index(s['sector'])
                    sector_icon = "★★★" if rank == 0 else "★★ " if rank == 1 else "★  "
                    sector_color, sector_name = Colors.GREEN, f"{sector_icon}{s['sector']}"
                elif s['sector'] in self.worst_sectors:
                    rank = self.worst_sectors.index(s['sector'])
                    sector_icon = "★★★" if rank == 0 else "★★ " if rank == 1 else "★  "
