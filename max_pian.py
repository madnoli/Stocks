...
class EnhancedOptionBuyerScanner:
    def __init__(self, mode='live', backtest_date=None):
        ...
        self.option_data = {}  # To store option chain summaries
    ...
    def fetch_option_chain(self, symbol):
        try:
            ...
            url = f"http://localhost:3000/api/equity/options/{clean_symbol}"
    ...
    def calculate_option_buyer_signals(self, symbol, timeframes_data):
        ...
        try:
            ...
            # Option chain based boost using PCR
            option_boost = 0
            oc = self.option_data.get(symbol)
            if oc:
                pcr = oc['pcr']
                if sector in self.best_sectors:
                    if pcr < 0.8:
    ...
    def enhanced_scan_cycle(self):
        ...
        # Build candidate stocks from all relevant sectors
        candidate_stocks = []
        for sector in set(self.best_sectors + self.worst_sectors):
            if sector in SECTOR_STOCKS:
                candidate_stocks.extend(SECTOR_STOCKS[sector])
        candidate_stocks = list(set(candidate_stocks))

        # Fetch option chain data in parallel
        def fetch_oc(symbol):
            if self.mode == 'backtest':
                ...
                return symbol, {'call_oi': call_oi, 'put_oi': put_oi, 'pcr': pcr, 'total_oi': total_oi}
            
            oc = self.fetch_option_chain(symbol)
            ...
            near_expiry = records['expiryDates'][0]
            call_oi = 0
            put_oi = 0
            for d in records.get('data', []):
                if d.get('expiryDate') == near_expiry:
                    if 'CE' in d:
                        call_oi += d['CE'].get('openInterest', 0)
                    if 'PE' in d:
                        put_oi += d['PE'].get('openInterest', 0)
            
            total_oi = call_oi + put_oi
            if total_oi == 0:
                return symbol, None
            
            pcr = put_oi / call_oi if call_oi > 0 else float('inf') if put_oi > 0 else 0
            return symbol, {'call_oi': call_oi, 'put_oi': put_oi, 'pcr': pcr, 'total_oi': total_oi}

        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(fetch_oc, symbol) for symbol in candidate_stocks]
            self.option_data = {}
            for future in as_completed(futures):
                sym, data = future.result()
                if data and data['total_oi'] > 5000:  # Liquidity threshold
                    self.option_data[sym] = data

        # Now build target_stocks_set using option data for accuracy
        target_stocks_set = set()

        # For best sectors (call focus): prefer low PCR
        for i, sector in enumerate(self.best_sectors):
            if sector in SECTOR_STOCKS:
                sector_stocks = SECTOR_STOCKS[sector]
                filtered = [s for s in sector_stocks if s in self.option_data and self.option_data[s]['pcr'] < 1.0]
                filtered.sort(key=lambda s: self.option_data[s]['total_oi'], reverse=True)
                num = 15 - i * 2  # More for top sectors
                target_stocks_set.update(filtered[:num])

        # For worst sectors (put focus): prefer high PCR
        ...
        if not target_stocks:
            print("No target stocks found after option chain filtering.")
            return

        print(f"Enhanced option buyer scanning {len(target_stocks)} stocks with Volume+OI analysis (filtered by option liquidity and PCR)")
        live_signals = []
        gapdown_filtered = 0
    ...
    def display_option_buyer_signals(self, signals, scan_time, gapdown_filtered, current_time):
        ...
        if not signals:
            ...
            call_table.add_column("PCR", justify="right", style="yellow")
            ...
            for s in call_signals[:15]:
                ...
                pcr_style = "green" if pcr_val is not None and pcr_val < 1 else "red" if pcr_val is not None and pcr_val > 1 else "white"
                pcr_str = f"[{pcr_style}]{pcr_str}[/]"
            ...
            put_table.add_column("PCR", justify="right", style="yellow")
