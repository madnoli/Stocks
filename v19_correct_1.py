# Enhanced Scanner for Option Buyers (v24-Compatible, TrueData, 5/15/30/60/1D, unified indicators, multi-session 10 rps, tz-aware, single progress bar)
import os
import logging
import warnings
...
# --- MODIFIED: Weights prioritizing short-term momentum for option buyers ---
# TIMEFRAME_WEIGHTS = {5: 2.0, 15: 1.8, 30: 1.2, 60: 0.8, 1440: 0.2}
# INDICATOR_WEIGHTS = {
#     # Key Ignition Indicators (Heavily Weighted)
#     "VolumeSurge": 3.5, "Momentum": 2.8, "ADX": 2.5, "ATR": 2.2, "ROC": 2.0,
#     # Confirmation Indicators (Moderately Weighted)
#     "RSI": 1.5, "MACD": 1.4, "EMA": 1.2, "VWAP": 1.2, "Bollinger": 2.5, # High weight for breakout
#     # Less Important for Ignition
#     "OBV": 1.0, "Stochastic": 0.8, "CCI": 0.8, "WWL": 0.7,
#     "MA": 0.5, # Long-term MAs are too slow for this purpose
# }
...
# Live settings
SCAN_SECONDS = int(os.getenv("SCAN_SECONDS", "60"))  # scan cadence
MARKET_START = "09:15"  # IST
MARKET_END   = "15:30"  # IST
INCLUDE_PREOPEN = False
MAX_BARS_KEEP = 1500
RUN_ONCE_AND_EXIT = False  # True to run one scan and exit
...
def calculate_rsi(df, period=14):
    ...
    rs.fillna(100, inplace=True) # If loss is 0, RSI is 100
    return 100 - (100 / (1 + rs))
...
# --- MODIFIED: Returns all 3 bands for squeeze calculation ---
...
# ---------------- Scoring (MODIFIED for Option Buyers) ----------------
def get_indicator_scores(df):
    scores = {}

    # --- MODIFIED: RSI score based on momentum crossings ---
    rsi_series = calculate_rsi(df)
    if len(rsi_series) > 1 and pd.notna(rsi_series.iloc[-1]):
        rsi = rsi_series.iloc[-1]
        prev_rsi = rsi_series.iloc[-2]
        if rsi > 60 and prev_rsi <= 60: scores['RSI'] = 2.0  # Strong bullish ignition
        elif rsi > 50 and prev_rsi <= 50: scores['RSI'] = 1.0
        elif rsi < 40 and prev_rsi >= 40: scores['RSI'] = -2.0 # Strong bearish ignition
        elif rsi < 50 and prev_rsi >= 50: scores['RSI'] = -1.0
        else: scores['RSI'] = 0.0
    else: scores['RSI'] = 0.0
    ...
    # --- MODIFIED: ADX score based on trend initiation ---
    ...
    # --- MODIFIED: Bollinger score based on Squeeze Breakout ---
    ...
    if len(bb_width) > 50 and pd.notna(bb_width.iloc[-1]):
        ...
        if not all(s.empty for s in [middle, upper, lower]):
            ...
            if is_in_squeeze and pd.notna(zscore) and zscore > 1.5:
                if close > upper.iloc[-1]: scores['Bollinger'] = 2.0  # Strong bullish breakout
                elif close < lower.iloc[-1]: scores['Bollinger'] = -2.0 # Strong bearish breakout
    ...
    if len(atr_val) >= 6 and all(pd.notna(val) for val in [atr_val.iloc[-1], atr_val.iloc[-5], df["Close"].iloc[-1], df["Close"].iloc[-5]]):
        atr_rising_sharply = (atr_val.iloc[-1] / atr_val.iloc[-5]) > 1.1 # 10% increase in ATR
    ...
    if len(mom) and pd.notna(mom.iloc[-1]):
        scores["Momentum"] = 1.5 if mom.iloc[-1] > 0.01 else (-1.5 if mom.iloc[-1] < -0.01 else 0.0) # Look for >1% move
...
def analyze_signals(timeframe_dataframes):
    ...
    for tf_min, df in timeframe_dataframes.items():
        ...
        for indicator, score in indicator_scores.items():
            ...
            # Max possible score is based on the highest potential score from our logic (e.g., 2.0 for RSI crossing)
...
def live_market_scanner_top20():
    ...
    # Read stocks list
    try:
        # IMPORTANT: Your 'shares.txt' should be filtered for liquid, F&O stocks for best results.
    ...
    while True:
        ...
        # Market hours gate (unless test mode)
        ...
        # (This section is simplified for brevity but works the same as the original)
        ...
        # Console output
        print("\n" + "="*92)
        print(f"| OPTION BUYER SCANNER | SIGNALS AT {now_ist.strftime('%Y-%m-%d %H:%M')} IST".center(100) + " |")
        print("="*92)
