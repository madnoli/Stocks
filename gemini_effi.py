...
class Enhanced3SectorScanner:
    ...
    # <<< --- OPTIMIZATION 1: NEW EFFICIENT DATA FETCHING FUNCTION --- >>>
    def fetch_and_resample_data(self, symbol):
        """
        Fetches 1-minute data once and resamples it to 5, 15, and 30-minute intervals.
        This reduces API calls by 67% for each stock.
        """
        try:
            ...
            # Make the single API call for 1-minute data
            raw_df = td_hist.get_historic_data(symbol, duration='5 D', bar_size='1 min')
            ...
            if df_1m is None or len(df_1m) < 30: # Need enough data to resample
                return None, False
            ...
            for tf in [5, 15, 30]:
                ...
                if len(df_resampled) >= 20:
                     resampled_data[tf] = df_resampled.tail(100)
    ...
    def enhanced_scan_cycle(self):
        ...
        print(f"⏰ Analyzing: {Colors.YELLOW}5min → 15min → 30min{Colors.RESET} (Resampled from 1-min data)")
        ...
        try:
            ...
            # <<< --- OPTIMIZATION 2: INCREASED THREAD COUNT --- >>>
            # Increased from 3 to 15. Safe now because the lock contention is gone.
            # Each worker makes one efficient call instead of three slow ones.
            with ThreadPoolExecutor(max_workers=15) as executor:
                
                # <<< --- OPTIMIZATION 3: SIMPLIFIED WORKER FUNCTION --- >>>
                def process_stock(symbol):
                    try:
                        # Single efficient call to fetch and resample all required timeframes
                        ...
                        if timeframes_data and len(timeframes_data) >= 2: # Require at least 2 TFs
