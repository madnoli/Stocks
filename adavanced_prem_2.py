            ...
            try:
                ...
                if len(timeframes_data) >= 3:
                    ...
                    if abs(score - 50) >= 12:
                        ...
                        # Calculate VolumeX (current volume / SMA20 volume)
                        if len(small_df) >= 20:
                        ...
                        # Add condition: only include if volume_x >= 2.0
                        if volume_x >= 2.0:
                            result = {
                                "symbol": symbol,
                                "signal": signal,
    ...
    def display_option_buyer_signals(self, signals, scan_time, gapdown_filtered, current_time):
        ...
        if not signals:
            ...
            for s in call_signals[:15]:
                ...
                volume_x = s.get('volume_x', 0)
                vol_x_str = f"{volume_x:.1f}x"
                vol_x_style = "green bold" if volume_x >= 2 else "white"
                vol_x_str = f"[{vol_x_style}]{vol_x_str}[/]"
