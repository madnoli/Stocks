...
def normalize_indicator_value(name, value):
    try:
        ...
        if name == "MACD": return 50 + max(-25, min(25, value / 10))
        ...
        if name == "ROC": return 50 + max(-25, min(25, value / 2))
        if name == "OBV": return 50 + max(-25, min(25, value))
        if name == "CCI": return max(0, min(100, (value + 200) / 4))
        if name == "WWL": return max(0, min(100, (value + 100)))
        if name == "ATR": return 50
        if name == "VolumeSurge": return max(0, min(100, value))
        if name == "Momentum": return max(0, min(100, value + 50))
        return 50
    except Exception:
        return 50
...
def pcr_bucket(pcr: float) -> str:
    if pcr is None or (isinstance(pcr, float) and np.isnan(pcr)): return "n/a"
    if pcr < 0.6: return "Ext Bearish"
    if pcr < 0.8: return "Bearish"
    if pcr <= 1.2: return "Neutral"
    if pcr <= 1.4: return "Bullish"
    return "Ext Bullish"

def spot_from_symbol(sym: str) -> str:
    map_overrides = {"RELIANCE-EQ": "RELIANCE", "RELIANCE-I": "RELIANCE"}
    return map_overrides.get(sym, sym).replace("-I","").replace("-II","")
...
def option_symbol_exists(sym: str) -> bool:
    try:
        now_ts = pd.Timestamp.now(tz="Asia/Kolkata")
        df = tdhist.get_series(sym, start_time=now_ts - pd.Timedelta(minutes=2),
                               end_time=now_ts, duration=pd.Timedelta(minutes=2), bar_size="1 min")
...
def build_option_symbol(underlying: str, expiry: str, strike: int, side: str) -> str:
    variants = [
        f"{EX_PREFIX_FO}{underlying}{expiry}{strike}{side}",
        f"{EX_PREFIX_FO}{underlying} {expiry} {strike} {side}",
        f"{EX_PREFIX_FO}{underlying}-{expiry}-{strike}-{side}",
    ]
    for s in variants:
        if option_symbol_exists(s):
            return s
    return variants[0]

def get_relevant_strikes(underlying: str, ltp: float):
    try:
        if ltp is None or (isinstance(ltp, float) and np.isnan(ltp)):
            center = 1000
            step = 100
        else:
        ...
        return list(range(800, 1300, 100))
...
def get_chain_totals_from_alt_api(underlying: str):
    return np.nan, np.nan

def get_option_chain_OI(underlying: str, timeframe_min: int, approx_ltp: float = None):
    pe_alt, ce_alt = get_chain_totals_from_alt_api(underlying)
    if not (np.isnan(pe_alt) or np.isnan(ce_alt)):
        return pe_alt, ce_alt, (pe_alt/ce_alt if ce_alt else np.nan)
...
def compute_multi_tf_pcr(underlying: str, approx_ltp: float = None):
    def only_pcr(x):
        try:
            ...
            return np.nan
    return {
        "5": only_pcr(get_option_chain_OI(underlying, 5, approx_ltp)),
        "15": only_pcr(get_option_chain_OI(underlying, 15, approx_ltp)),
        "30": only_pcr(get_option_chain_OI(underlying, 30, approx_ltp)),
        "60": only_pcr(get_option_chain_OI(underlying, 60, approx_ltp)),
    }
...
class EnhancedScannerPCR:
    ...
    def update_sectors_from_api(self):
        try:
            ...
            if isinstance(data, dict):
                for k in ("data","indices","results","payload"):
            ...
            for row in data:
                if not isinstance(row, dict):
                    continue
                name = None
                for f in ("name","indexName","index","symbol","idxName"):
                ...
                if not name or name not in NSE_INDEX_TO_SECTOR:
                    continue
                chg = None
                for f in ("pChange","percentChange","changePercent","changepercent","pchg","chgrate"):
                    if f in row and row[f] is not None:
                        try:
                            ...
                            pass
                if chg is None:
                    last = row.get("last") or row.get("ltp") or row.get("close")
                    prev = row.get("prevClose") or row.get("previousClose")
                    try:
                        if last is not None and prev is not None:
                            chg = (float(last)/float(prev)-1)*100.0
                    except Exception:
                        pass
                if chg is None:
                    continue
            ...
            self.best_sectors = [p["sector"] for p in perf[:4]]
            self.worst_sectors = [p["sector"] for p in perf[-4:]][::-1]
            return True
        except Exception as e:
            logger.warning(f"Sector API error: {e}")
            return False
    ...
    def normalize_live_data(self, df, symbol):
        try:
            ...
            for c in dfc.columns:
                lc = str(c).lower()
                if lc in ("timestamp","time","date"): rename[c] = "Date"
    ...
    def score_symbol(self, symbol, tf_data):
        try:
            if not tf_data:
                return "Neutral", 0
            sector = next((s for s,l in SECTOR_STOCKS.items() if symbol in l), None)
            if not sector:
                return "Neutral", 0
            ...
            for tf, df in tf_data.items():
                ...
                tf_score = 0.0; tf_weight_sum = 0.0
                cp = df["Close"].iloc[-1]
                    ...
                    if name in indicators and indicators[name] is not None and not indicators[name].empty:
                        ...
                        if name in ("MA","EMA","VWAP"):
                            ...
                            if pd.isna(base) or base == 0:
                                ...
                                norm = 75 if diff >= 2 else 60 if diff >= 0 else 50 if diff >= -2 else 40 if diff >= -5 else 25
                        else:
                            norm = normalize_indicator_value(name, v)
                        tf_score += norm * wt
                        tf_weight_sum += wt
                if tf_weight_sum <= 0: continue
                tf_final = tf_score / tf_weight_sum
                tf_scores[tf] = tf_final
                mult = TIMEFRAME_WEIGHTS.get(tf, 1.0)
                total_ws += tf_final * mult; total_w += mult
    ...
    def enrich_tape(self, symbol):
        try:
            ...
            close_col = cols.get("close") or cols.get("ltp") or cols.get("last") or list(df.columns)[-1]
            vol_col = cols.get("volume") or cols.get("vol") or cols.get("qty")
            ltp = float(df[close_col].iloc[-1]) if close_col else np.nan
            if "buy_volume" in cols and "sell_volume" in cols:
                byv = float(df[cols["buy_volume"]].iloc[-1]); selv = float(df[cols["sell_volume"]].iloc[-1])
            else:
                if vol_col and close_col and len(df) >= 2:
                    v = float(df[vol_col].iloc[-1]); prev_close = float(df[close_col].iloc[-2]); last_close = float(df[close_col].iloc[-1])
                    byv = v if last_close >= prev_close else 0.0; selv = v - byv
                else:
                    byv, selv = np.nan, np.nan
    ...
    def enhanced_scan_cycle(self):
        ...
        def work(sym):
            ...
            for tf in [5,15,30,60,"daily"]:
                ...
                if df is not None:
                    tf_data[tf] = df
            if len(tf_data) < 3: return None
            signal, score = self.score_symbol(sym, tf_data)
            if abs(score - 50) < 15: return None
            sector = next((s for s,l in SECTOR_STOCKS.items() if sym in l), "NA")
            ...
            row = {
                "symbol": sym, "sector": sector, "signal": signal, "score": score,
                "timeframes": len(tf_data), "tfdetails": list(tf_data.keys()),
                "ltp": ltp, "by_volume": byv, "sell_volume": selv, "delta_volume": dvol,
                "pcr_5": pcrs.get("5"), "pcr_15": pcrs.get("15"), "pcr_30": pcrs.get("30"), "pcr_60": pcrs.get("60"),
            }
            prev = self.last_cycle_scores.get(sym)
            row["score_delta"] = "n/a" if prev is None else f"{score - prev:+.1f}"
            pref = row["pcr_15"] if row["pcr_15"] is not None and not (isinstance(row["pcr_15"], float) and np.isnan(row["pcr_15"])) else row["pcr_30"]
            row["pcr_remark"] = pcr_bucket(pref)
        ...
        if bullish or bearish:
            any_row = (bullish or bearish)[0]
            root = spot_from_symbol(any_row["symbol"])
            exp = get_current_expiries_for_underlying(root)[0]
            ltp_dbg = any_row.get("ltp")
            if isinstance(ltp_dbg, (int,float)) and not np.isnan(ltp_dbg):
                step = 50 if ltp_dbg <= 500 else 100
                strike_guess = int(round(ltp_dbg / step)) * step
            else:
                step = 100
                strike_guess = 1000
            test_ce = build_option_symbol(root, exp, strike_guess, "CE")
            print(f"Debug option symbol: {test_ce}")
