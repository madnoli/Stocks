...
DURATION_DAILY = os.getenv("DURATION_DAILY", "730 D")  # ample for long lookbacks and backtests [web:13]
...
# ---------------- CLI ----------------
def parse_args():
    p = argparse.ArgumentParser(description="Single-run swing scanner with backdated as-of support (IST).")
    p.add_argument("--asof", type=str, default=None, help="Backdate as-of in YYYY-MM-DD (IST). Uses completed W/M/3M/6M bars up to this date.")
    p.add_argument("--shares", type=str, default="shares.txt", help="Path to shares list file (one symbol per line).")
    p.add_argument("--threads", type=int, default=64, help="Thread pool size for fetching.")
    p.add_argument("--outfile_prefix", type=str, default="", help="Optional prefix for output filename.")
    return p.parse_args()  # [web:49]
...
def resample_ohlcv(df: pd.DataFrame, rule: str) -> pd.DataFrame:
    ...
    return out  # [web:13][web:35][web:36]
...
def fetch_daily(symbol_orig, limiter, hist):
    ...
    try:
        t0 = time.time()
        limiter.acquire()
        # Request EOD/daily bars; duration accepts D/W/M/Y formats
...
# ---------------- Single-run swing scan with backdate ----------------
def run_single_swing_scan(asof_str=None, shares_path="shares.txt", threads=64, outfile_prefix=""):
    ...
    try:
        ...
        logger.info(f"Loaded {len(stocks)} stocks from {shares_path}. Ensure these are liquid F&O stocks.")
    except Exception as e:
        logger.error(f"Could not read {shares_path}: {e}")
        return

    # Prefetch daily and build swing TFs
    stock_multi_data = prefetch_all(stocks, max_workers=threads)
    if not stock_multi_data:
        print("No stocks with sufficient data found on initial load. Exiting.")
        return

    # Use backdated cutoff if provided, else now
    if asof_str:
        # interpret as local IST date end-of-day; tz-aware throughout avoids slicing issues
        asof_dt = pd.to_datetime(asof_str)
        if asof_dt.tzinfo is None:
            asof_dt = asof_dt.tz_localize(IST)
        else:
            asof_dt = asof_dt.tz_convert(IST)
        now_ist = asof_dt
    else:
        now_ist = datetime.now(IST)

    date_tag = now_ist.strftime("%Y-%m-%d")
    output_filename = f"{outfile_prefix}{date_tag}_options_scan_results.txt" if outfile_prefix else f"{date_tag}_options_scan_results.txt"

    signals_this_scan = []
    current_scores = {}

    # Completed periods only from resample; we slice to cutoff instant
