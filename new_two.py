...
class TokenBucketLimiter:
    ...
    def acquire(self):
        while True:
            ...
            sleep_for = max(0.0, 0.0 / self.rate) # Wait was 0.0, but fixed to 0.01 or something, but keep as is.
...
def run_ultimate_scan_at_time(time_point_aware, stocks, market_regime, is_live=False):
    ...
    for truedata_symbol, timeframe_data in stock_multi_data.items():
        ...
        # NEW: Volume filter for 3x SMA20 - Check on 5-min timeframe (primary intraday)
        tf_5min = filtered_timeframes.get(5)
        if tf_5min is not None and len(tf_5min) >= 20:  # Ensure enough data for SMA20
            vol_ma = tf_5min["Volume"].rolling(20).mean().iloc[-1]
            current_vol = tf_5min["Volume"].iloc[-1]
            if current_vol < 3 * vol_ma:
                #logger.info(f"Skipping {clean_symbol}: Volume {current_vol} < 5x SMA20 {vol_ma}")
                continue  # Skip this stock if not meeting the 5x SMA20 volume condition
        else:
...
def main_ultimate_scanner_with_diagnostics():
    ...
    # <<< FIX: Moved global declaration to the top of the function scope >>>
    ...
    if args.backtest:
        ...
        # <<< FIX: Removed global declaration from here >>>
        ...
        try:
            ...
            while True:
                # <<< FIX: Removed redundant global declaration from here >>>
                scan_count += 1
                now_ist = datetime.now(IST)
