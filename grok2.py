...
CONFIG = {
    ...
    # Market times (IST)
    "MARKET_START": "09:15",
    "FIRST_RUN_AT": "09:20",  # first 5-minute close
    "MARKET_END": "15:30",
    ...
    "LOG_LEVEL": os.getenv("LOG_LEVEL", "ERROR"),  # DEBUG/INFO/WARNING/ERROR/CRITICAL
    ...
    "INDICATOR_PERIODS": {
        ...
        "ADL_LOOKBACK": 10,
        "REL_VOL": 50,
        "VWAP_REGIME": None,   # Intentionally None; handled safely
        "OBV_CONFIRM": 5,
        "OI_SURGE": 20,
        "OI_MOMENTUM": 10,
    },
    "INDICATOR_WEIGHTS": {
        ...
        # OI emphasis for option buyers
        "OISurge": 2.2, "OIMomentum": 2.0, "CallBias": 2.5, "PutBias": 2.5
    },
    # Intraday emphasis for option buyers
...
def get_indicator_scores(df):
    ...
    if oi_col is not None:
        ...
        vdist = ((df_oi["Close"] - vwap_day) / vwap_day.replace(0, np.nan))
        ...
        if conds_ok and pd.notna(oi_z.iloc[-1]) and pd.notna(oi_mom.iloc[-1]) and pd.notna(rel_vol.iloc[-1]) and pd.notna(vdist.iloc[-1]) and pd.notna(adx_val.iloc[-1]):
            ...
            # Continuation conditions aligned for option buying
            ...
            # Traps: reduce conviction where participation is weak
    ...
    max_score = max(abs(s) for s in scores.values() if s != 0) if any(scores.values()) else 1.0
    if max_score == 0:
        max_score = 1.0
    for k in scores:
...
def prefetch_all(stocks, max_workers=CONFIG["MAX_WORKERS"]):
    ...
    progress_kwargs = dict(total=total_calls, desc="Prefetching Data", ncols=100, disable=not CONFIG["SHOW_PROGRESS"])
...
# ---------------- Main ----------------
def main():
    parser = argparse.ArgumentParser(description="Options buyer scanner with OI+Volume fusion, speed-optimized fetch, and backtest modes")
