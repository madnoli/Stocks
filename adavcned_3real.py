...
# ENHANCED REAL-TIME 3-SECTOR SCANNER WITH FIXED API SECTORAL DATA
# Features: ATR, Volume Surge, Momentum + Enhanced Weights + API Sectoral Data
import pandas as pd
import numpy as np
...
class Enhanced3SectorScanner:
    ...
    def fetch_live_data(self, symbol, timeframe):
        """Fetch live data with gap-down detection"""
        try:
            tf_map = {
                5: '5 min', 
                15: '15 min', 
    ...
    def calculate_enhanced_signals(self, symbol, timeframes_data):
        ...
        try:
            ...
            # Enhanced 3-sector boost system
            sector_boost = 0
    ...
    def enhanced_scan_cycle(self):
        ...
        print(f"\n{Colors.CYAN}🔄 Starting ENHANCED 3-sector scan at {current_time.strftime('%H:%M:%S')}{Colors.RESET}")
        ...
        try:
            # Get target stocks from top 3 best and top 3 worst sectors
            ...
            print(f"🎯 Enhanced scanning {len(target_stocks)} stocks from 6 sectors")
    ...
    def display_enhanced_signals(self, signals, scan_time, gap_down_filtered):
        ...
        print(f"{Colors.BLUE}💡 Enhanced 3-sector strategy with Volume Surge & Momentum + LIVE API sectoral data{Colors.RESET}")
...
def main():
    ...
    print(f"{Colors.CYAN}🔧 Features: 3 Best + 3 Worst sectors, Enhanced indicators, API sectoral data{Colors.RESET}")
