...
# Enhanced filtering parameters - UPDATED VOLUME SPIKE TO 5X
VOLUME_SPIKE_X = 5.0             # 5X VOLUME THRESHOLD AS REQUESTED
...
def get_market_session_context():
    ...
    if 9 <= hour <= 10:  # Opening hour
        return {
            'session': 'OPENING',
            'volume_multiplier': 0.8,    # Easier to achieve 5x in opening
            'oi_multiplier': 0.9,
        ...
        return {
            'session': 'MID_MORNING', 
            'volume_multiplier': 0.9,    # Slightly easier for 5x requirement
            'oi_multiplier': 1.0,
        ...
        return {
            'session': 'CLOSING',
            'volume_multiplier': 0.7,    # Much easier near close for 5x
            'oi_multiplier': 0.8, 
        ...
        return {
            'session': 'NORMAL',
            'volume_multiplier': 1.0,    # Standard 5x requirement
            'oi_multiplier': 1.0,
...
def enhanced_volume_signal(spot_price, hist_avg_volume, recent_volume, price_change_pct, session_context):
    """Enhanced volume analysis with 5X threshold and session context"""
    if not hist_avg_volume or hist_avg_volume <= 0:
        hist_avg_volume = max(recent_volume * 0.5, 1000)  # Fallback estimate

    # Apply session-specific multipliers for 5X requirement
    adjusted_threshold = VOLUME_SPIKE_X * session_context['volume_multiplier']  # 5.0 * session_multiplier
    ...
    # Volume significance - MUST BE 5X+ after session adjustment
    volume_significant = vol_ratio >= adjusted_threshold
    # Price momentum confirmation
    momentum_confirmed = price_momentum_abs >= adjusted_momentum
    # Combined signal - BOTH volume spike AND price momentum required
    ...
    return {
        ...
        'session_multiplier': session_context['volume_multiplier']
    }
...
def fetch_symbol_metrics(symbol, cache):
    ...
    # Enhanced scoring system with higher weight on volume quality for 5X requirement
    metrics_for_score = {
        'institutional_flow': min(1.0, smart_money_avg / 3.0),
        'gamma_exposure': min(1.0, abs(gex) / (abs(gex) + 100.0)) if gex else 0.0,
        'volume_quality': 1.0 if vol_sig['valid_signal'] else 0.0,  # Binary for 5X requirement
    ...
    # Adjusted weights - higher emphasis on volume quality for 5X spike detection
    weights = {
        'institutional_flow': 0.20,
        'gamma_exposure': 0.15,
        'volume_quality': 0.30,      # Increased weight for 5X volume requirement
    ...
    # Enhanced classification - stricter due to 5X requirement
    if total_score >= 0.8:
...
def fetch_symbol_metrics_debug(symbol, cache):
    """Debug wrapper for symbol metrics with 5X focus"""
    try:
        ...
        if DEBUG_MODE:
            ...
            console.print(f"  🌊 Vol Ratio: {metrics.get('Vol Ratio', 0):.2f}x {'🔥' if metrics.get('Vol Ratio', 0) >= 5.0 else '❄️'}")
            console.print(f"  🎪 Session: {metrics.get('Session', 'N/A')}")

            # Show 5X volume threshold status
            session_ctx = get_market_session_context()
            effective_threshold = VOLUME_SPIKE_X * session_ctx['volume_multiplier']
            vol_ratio = metrics.get('Vol Ratio', 0)
            volume_status = "✅ 5X+" if vol_ratio >= effective_threshold else f"❌ Need {effective_threshold:.1f}x"
            console.print(f"  🚀 5X Volume: {volume_status} (Current: {vol_ratio:.1f}x)")
...
def compute_cycle(symbols, cache):
    ...
    console.print(f"🎯 5X Volume Threshold: {VOLUME_SPIKE_X * session_ctx['volume_multiplier']:.1f}x (Base: {VOLUME_SPIKE_X}x)")
    ...
    for i, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            if metrics:
                ...
                # Additional 5X volume check
                volume_spike_ok = vol_ratio >= effective_threshold

                if filter_result['passes'] and volume_spike_ok:
                    results.append(metrics)
                    console.print(f" ✅ (Vol: {vol_ratio:.1f}x)")
                elif not volume_spike_ok:
                    console.print(f" 🌊 (Vol: {vol_ratio:.1f}x < {effective_threshold:.1f}x required)")
                else:
    ...
    valid_results = [r for r in results if r.get('Score') and r.get('Vol Ratio', 0) >= VOLUME_SPIKE_X * session_ctx['volume_multiplier']]
    console.print(f"\n📊 Found {len(valid_results)} stocks with 5X+ volume spikes out of {len(symbols)} processed")
...
def render_tables(df):
    ...
    # Filter valid stocks only - must have 5X+ volume spike
    session_ctx = get_market_session_context()
    effective_threshold = VOLUME_SPIKE_X * session_ctx['volume_multiplier']
    ...
    valid_df = df[
        df["Score"].notna() & 
        (df["Score"] > 0) & 
        (df["Vol Ratio"] >= effective_threshold)
    ].copy()

    if valid_df.empty:
        console.print(f"[yellow]📊 No stocks found with required {effective_threshold:.1f}x volume spike[/yellow]")
        total_stocks = len(df[df["Score"].notna()])
        if total_stocks > 0:
            max_vol_ratio = df["Vol Ratio"].max()
            console.print(f"[yellow]    • {total_stocks} stocks processed, highest volume ratio: {max_vol_ratio:.1f}x[/yellow]")
        return

    # Categorize stocks based on stricter 5X criteria
    def get_category(row):
        remark = str(row.get("Remark", ""))
        score = row.get("Score", 0)
        vol_ratio = row.get("Vol Ratio", 0)

        if "STRONG_BUY" in remark and vol_ratio >= 10.0:  # 10X+ volume
            return "🔥 Super Strong"
        elif "STRONG_BUY" in remark or score >= 0.8:
            return "🚀 Strong Buy"
        elif "BUY" in remark or score >= 0.65:
            return "✅ Buy"
        elif "WATCH" in remark or score >= 0.5:
            return "👀 Watch"
        else:
            return "📊 Monitor"
    ...
    # Sort by volume ratio first (5X+ requirement), then score
    valid_df = valid_df.sort_values(["Vol Ratio", "Score"], ascending=[False, False])

    # Take top performers
    display_df = valid_df.head(20)

    console.print(f"\n🎯 Found {len(valid_df)} stocks with 5X+ volume spikes:")
    ...
    # Display columns
    DISPLAY_COLS = [
        "Stock", "Category", "Vol Ratio", "Score", "Price", "PCR", "V/OI Ratio",
    ...
    try:
        gt = (
            GT(table_df)
            .tab_header(
                title="🔥 5X Volume Spike Scanner",
                subtitle=f"Option Buyer Targets • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} • {len(valid_df)} opportunities with 5X+ volume"
            )
            .fmt_number(columns=["Volume", "OI"], decimals=0, use_seps=True)
            .fmt_number(columns=["Price", "PCR", "V/OI Ratio", "Score", "Smart Flow", "Liq Score"], decimals=2)
            .fmt_number(columns=["Vol Ratio"], decimals=1)
    ...
    # Console Table
    table = Table(title="🔥 5X Volume Spike Targets", show_lines=True, expand=True)
    ...
    for _, row in table_df.iterrows():
        # Style based on volume ratio
        vol_ratio = row.get("Vol Ratio", 0)
        if vol_ratio >= 10.0:
            style = "bold red"  # 10X+ super hot
        elif vol_ratio >= 7.5:
            style = "bold green"  # 7.5X+ strong
        elif vol_ratio >= 5.0:
            style = "green"  # 5X+ good
        else:
            style = "white"
        ...
        for col_name in table_df.columns:
            ...
            if pd.isna(val) or val is None:
                ...
                formatted = f"{val:,.0f}"
            elif col_name in ["Score", "PCR", "V/OI Ratio", "Smart Flow", "Liq Score"]:
                formatted = f"{val:.2f}"
            elif col_name == "Vol Ratio":
        ...
        if len(row_data) >= 3:
            ...
            row_data[2] = f"[{style}]{row_data[2]}[/{style}]"  # Vol Ratio

        table.add_row(*row_data)

    console.print(table)

    # Enhanced summary statistics for 5X focus
    if len(valid_df) > 0:
        console.print(f"\n📈 5X Volume Spike Analysis:")
        console.print(f"  • Average Volume Ratio: {valid_df['Vol Ratio'].mean():.1f}x")
        console.print(f"  • Highest Volume Ratio: {valid_df['Vol Ratio'].max():.1f}x")
        ...
        super_spikes = len(valid_df[valid_df['Vol Ratio'] >= 10.0])
        if super_spikes > 0:
            console.print(f"  • 🔥 Super Spikes (10X+): {super_spikes} stocks")
...
def run_once():
    ...
    # Show session info with 5X focus
    session_ctx = get_market_session_context()
    effective_5x_threshold = VOLUME_SPIKE_X * session_ctx['volume_multiplier']
    console.print(f"\n🕐 Market Session: {session_ctx['session']} (Weight: {session_ctx['weight']:.1f}x)")
    console.print(f"🔥 5X Volume Requirement: {effective_5x_threshold:.1f}x (Base: {VOLUME_SPIKE_X}x)")
    ...
    console.print(f"\n✅ 5X Volume Spike Scan completed at {datetime.now().strftime('%H:%M:%S')}")

def main():
    """Main execution function"""
    console.print("\n" + "="*65)
    console.print("🔥 5X VOLUME SPIKE OPTION SCANNER v2.1")
    console.print("    Advanced 5X+ Volume Detection & Smart Money Flow")
    console.print("="*65)

    # Configuration summary with 5X emphasis
    console.print(f"\n⚙️  Configuration:")
    console.print(f"   • 🔥 Volume Spike Requirement: {VOLUME_SPIKE_X}x minimum")
    ...
    session_ctx = get_market_session_context()
    effective_threshold = VOLUME_SPIKE_X * session_ctx['volume_multiplier']
    console.print(f"\n🎯 Current Session: {session_ctx['session']}")
    console.print(f"🔥 Effective 5X Threshold: {effective_threshold:.1f}x")

    console.print(f"\n⏰ Starting 5X volume spike scan at {datetime.now().strftime('%H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            console.rule(f"[bold blue]🔥 5X Volume Spike Scan: {ts}[/bold blue]")

            run_once()

            ts_end = datetime.now().strftime('%H:%M:%S')
            console.rule(f"[bold green]✅ Scan Complete: {ts_end}[/bold green]")
            console.print(f"😴 Sleeping for {SLEEP_SECONDS} seconds...\n")

        except KeyboardInterrupt:
            console.print("\n[yellow]🛑 5X Volume Scanner stopped by user[/yellow]")
            break
        except Exception as e:
