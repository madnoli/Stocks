...
def add_buy_sell_delta_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Intrabar Buying/Selling Pressure split using OHLCV:
      - up_pressure = (Close - Low) / (High - Low)
      - down_pressure = (High - Close) / (High - Low)
      - handle zero range by equal split 50/50
      - in ties, bias slightly by direction of Close vs previous Close
    Produces:
      BuyVol, SellVol, DeltaVol = BuyVol - SellVol
    """
    ...
    # On ties, bias 60/40 towards direction; neutral stays 50/50
    ...
    return df  # Using OHLCV-based delta split consistent with intrabar pressure method [web:1][web:8][web:5]
...
def get_option_chain(symbol: str):
    """
    Replace with actual broker/exchange API returning current option chain.
    For TrueData users, consider truedata-ws option chain streaming per docs (fields include oi for CE/PE).
    Return a list of rows, each row may be flat or contain nested CE/PE dicts.
    """
    # Example placeholder: try a local microservice; else return None
    # r = requests.get(f"http://localhost:3002/api/optionChain?symbol={symbol}", timeout=10)
    # return r.json()
    return None  # Implement with your live source for production [web:26][web:29]

def compute_pcr_from_option_chain(option_chain) -> float | None:
    """
    Aggregates Put OI and Call OI across the chain and returns PCR = PutOI / CallOI.
    Supports flat fields like put_oi/call_oi and nested row['PE']['oi']/row['CE']['oi'].
    """
    try:
        ...
        if total_call_oi <= 0:
            return None
        return total_put_oi / total_call_oi  # PCR(OI) standard definition [web:36][web:15][web:18]
    except Exception as e:
...
class Enhanced3SectorScanner:
    def __init__(self):
        ...
        # Sector snapshots for momentum deltas
        self.sector_snapshots = []   # {"timestamp": dt, "sector": str, "change": float}
    ...
    def _trim_sector_snapshots(self):
        try:
            ...
            logger.warning(f"Trim snapshots error: {e}")
    ...
    def fetch_live_sectoral_performance(self):
        try:
            ...
            if not sectoral:
                print("No mapped sectors from API.")
                return False
    ...
    def _print_top_sector_delta_table(self, sectoral):
        try:
            ...
            logger.warning(f"Momentum table error: {e}")
    ...
    def normalize_live_data(self, df, symbol):
        try:
            ...
            if len(d) < 20:
                return None
            # Add Buy/Sell/Delta columns for volume
            try:
    ...
    def calculate_enhanced_signals(self, symbol, timeframes_data):
        try:
            ...
            try:
                ...
                # sector deltas
                if RELAXED_MODE:
                    d5_ok = (d5 is None) or (d5 > 0)
                    d15_ok = (d15 is None) or (d15 > 0)
                    d5_ok_s = (d5 is None) or (d5 < 0)
                    d15_ok_s = (d15 is None) or (d15 < 0)
                else:
                    d5_ok = (d5 is not None and d5 > 0)
                    d15_ok = (d15 is not None and d15 > 0)
                    d5_ok_s = (d5 is not None and d5 < 0)
                    d15_ok_s = (d15 is not None and d15 < 0)
            ...
            try:
                ...
                if len(tfs) >= 3:
                    ...
                    if abs(score - 50) >= 15:
                        ...
                        # Optionally show latest bar delta from shortest TF
                        if 5 in tfs:
                            last_bar = tfs[5].iloc[-1]
                        else:
                            last_tf = sorted([k for k in tfs.keys() if isinstance(k, (int, float))])[0]
                            last_bar = tfs[last_tf].iloc[-1]
                        ...
                        return {"symbol": sym, "signal": sig, "score": score, "sector": sector,
                                "tfcount": len(tfs), "time": datetime.now(), "pcr": pcr_val,
    ...
    def display_signals(self, signals, scan_time):
        ...
        for s in bulls[:15]:
            ...
            print(f"{s['symbol']:<10} {s['sector']:<16} {s['signal']:<18} {s['score']:>6.1f} ({s['tfcount']} TFs){pcr_str}{dstr}")
...
def main():
    ...
    # One immediate cycle for visibility
    try:
