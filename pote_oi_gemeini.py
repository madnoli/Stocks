# ==============================================================================
# ENHANCED OPTION BUYER SCANNER v3.2 - COMPLETE FINAL FIXED VERSION
# Fixed: Great Tables + 5-Minute Volume/OI Tracking + All Features
# ==============================================================================
...
def calculate_5min_volume_oi_changes(df, symbol, scan_time):
    ...
    try:
        # Get 5-minute data up to scan time
        df_5min = df[df.index <= scan_time]
        
        if len(df_5min) < 2:
            return 0, 0, 0, 0  # current_vol, current_oi, vol_change_pct, oi_change_pct
        
        # Current 5-minute candle data
        current_volume = int(df_5min['Volume'].iloc[-1])
        current_oi = int(df_5min.get('OpenInterest', df_5min['Volume'] * 0.3).iloc[-1])
        
        # Previous 5-minute candle data (1 candle back)
...
def extract_5min_volume_oi_data(df, symbol, time_point=None, is_live=False):
    ...
    try:
        ...
        # Update intraday tracking for next scan
        ...
        if abs(vol_change_pct) > 0.1:  # Only show if meaningful change
            volume_change_legacy = f"{vol_change_pct:+.1f}%"
        else:
            volume_change_legacy = "N/A"
            
        if abs(oi_change_pct) > 0.1:  # Only show if meaningful change
            oi_change_legacy = f"{oi_change_pct:+.1f}%"
        else:
            oi_change_legacy = "N/A"
...
def generate_backtest_timestamps(backtest_date):
    ...
    # Start from 09:15, first scan at 09:20:15 (after 09:15-09:20 candle)
    ...
    # First scan at 09:20:15 (after 09:15-09:20 candle closes)
    first_scan = current_time + timedelta(minutes=5, seconds=15)
    timestamps.append(first_scan)
    
    # Generate subsequent 5-minute scans
...
def run_scan_at_time_5min_fixed(time_point_aware, stocks, market_regime, is_live=False):
    """FIXED: Core logic with proper 5-minute Volume/OI tracking."""
    if is_live:
    ...
    for symbol, timeframe_data in stock_multi_data.items():
        ...
        if abs(score) >= Config.SCORE_THRESHOLD_MIN:
            ...
            # FIXED: Use 5-minute timeframe specifically for volume/OI tracking
            tf_5min = filtered_timeframes.get(5)  # Get 5-minute timeframe
            if tf_5min is not None:
                oi_vol_data = extract_5min_volume_oi_data(tf_5min, clean_symbol, time_point_aware, is_live=is_live)
            else:
                # Fallback to other timeframes if 5-min not available
