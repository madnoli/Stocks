...
# MODIFIED: Finds explosive breakout candidates for options trading.
import pandas as pd
import numpy as np
...
try:
    ...
    td_hist = None # Set to None to handle initialization failure
...
class OptionsBreakoutScanner:
    ...
    def show_initialization_status(self):
        print(f"\n{Colors.CYAN}{Colors.BOLD}🚀 OPTIONS BREAKOUT SCANNER (VOLATILITY & MOMENTUM FOCUSED){Colors.RESET}")
        ...
        print(f"\n{Colors.YELLOW}🔄 Running initial setup...{Colors.RESET}")
    ...
    def normalize_live_data(self, df, symbol):
        try:
            ...
            # <<< --- BUG FIX: CHECK FOR 'timestamp' OR 'time' --- >>>
            ...
            if not all([date_col, open_col, high_col, low_col, close_col, vol_col]):
                logger.error(f"Missing required columns for {symbol}. Available columns: {list(df.columns)}")
                return None
    ...
    def fetch_and_resample_data(self, symbol):
        if not td_hist:
            logger.error("Truedata client not initialized. Cannot fetch data.")
            return None, False
        try:
            with self.api_lock:
            ...
            raw_df = td_hist.get_historic_data(symbol, duration='10 D', bar_size='1 min')
            if raw_df is None or raw_df.empty: return None, False

            df_1m = self.normalize_live_data(raw_df, symbol)
            if df_1m is None or len(df_1m) < 60: return None, False
            ...
            for tf in [5, 15, 30, 60]:
                ...
                if len(df_resampled) >= 20: resampled_data[tf] = df_resampled.tail(150)
            return resampled_data, is_gap_down
        except Exception as e:
            logger.error(f"Data fetch/resample error for {symbol}: {e}")
            return None, False
    ...
    def calculate_options_signals(self, symbol, timeframes_data):
        ...
        try:
            for tf in [15, 30, 60]:
                ...
                is_firing_up = latest.get('squeeze_fire_up', False)
                is_firing_down = latest.get('squeeze_fire_down', False)
                vol_surge_score = latest.get('Volume_Surge', 0)
                momentum_score = latest.get('Momentum', 50)
                atr_accel = latest.get('ATR_accel', 0) > 0

                sector = next((s for s, st in SECTOR_STOCKS.items() if symbol in st), 'N/A')
                sector_boost = 0
                if sector in self.best_sectors: sector_boost = 5
                if sector in self.worst_sectors: sector_boost = -5
    ...
    def process_stock(self, symbol):
        try:
            timeframes_data, has_gap_down = self.fetch_and_resample_data(symbol)
            if has_gap_down: return None, True
