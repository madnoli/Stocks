    ...
    try:
        ...
        for item in data:
            row = {
                ...
                'Vol_Multi': item.get('vol_multi', 0),
        ...
        gt_table = (
            ...
            .tab_spanner(label="Current Data", columns=["Curr_Vol", "Curr_OI", "Vol_Change", "OI_Change", "Vol_Multi"])
            .tab_spanner(label="Analysis", columns=["Flow", "Action"])
            .fmt_number(columns=["Score", "Trend", "Momentum", "Volume", "OI", "Vol_Multi"], decimals=2)
            ...
            .data_color(columns=["Vol_Multi"], palette=["white", "green"], domain=[0, 5])
        ...
        for i, row in df.iterrows():
            ...
            print_colored(f"{marker}{row['Stock']:<12} | {row['Signal']:<16} | {row['Score']:>7.2f} | {row['Curr_Vol']:>10} | {row['Curr_OI']:>10} | {vol_chg_str:>7} | {oi_chg_str:>7} | {row['Vol_Multi']:>7.1f}x | {row['Action']:<14}", color)
        print("="*120)
...
def create_rich_enhanced_table(data, title, new_stocks=None, show_time=None):
    ...
    if RICH_AVAILABLE:
        ...
        table.add_column("Vol Multi", style="bright_green", width=8, justify="right")
        ...
        for item in data:
            ...
            vol_multi_raw = item.get('vol_multi', 0)
            if vol_multi_raw >= 2.5:
                vol_multi_style = f"[bright_green]{vol_multi_raw:.1f}x[/bright_green]"
            else:
                vol_multi_style = f"[dim]{vol_multi_raw:.1f}x[/dim]"
...
def create_enhanced_ascii_table(data, title, new_stocks=None, show_time=None):
    ...
    header = (f"{'Stock':<12} | {'Signal':<16} | {'Score':>8} | {'Trend':>7} | {'Mom':>9} | {'Vol':>7} | {'OI':>7} | "
              f"{'CurrVol':>10} | {'CurrOI':>10} | {'VolΔ%':>8} | {'OIΔ%':>8} | {'Vol Multi':>8} | {'Flow':<18} | {'Action':<14}")
    ...
    for item in data:
        ...
        vol_chg_str = f"{vol_chg:+.1f}" if isinstance(vol_chg, (int, float)) and abs(vol_chg) > 0.1 else "N/A"
        oi_chg_str = f"{oi_chg:+.1f}" if isinstance(oi_chg, (int, float)) and abs(oi_chg) > 0.1 else "N/A"
        vol_multi_str = f"{vol_multi:.1f}x" if vol_multi > 0 else "N/A"
        ...
        row = (f"{symbol:<12} | {item['signal']:<16} | {item['score']:>8.2f} | {item['sub_scores'].get('Trend', 0):>7.2f} | "
               ...
               f"{oi_chg_str:>7}% | {vol_multi_str:>8} | {item.get('flow', 'Unknown'):<18} | {item.get('action', 'Consider'):<14}")
...
def create_compact_backtest_table(data, title, new_stocks=None, show_time=None):
    ...
    if GREAT_TABLES_AVAILABLE:
        ...
        table.add_column("Vol Multi", style="bright_green", width=7, justify="right")
        ...
        for i, item in enumerate(data, 1):
            ...
            vol_multi_raw = item.get('vol_multi', 0)
            vol_multi_display = f"[bright_green]{vol_multi_raw:.1f}x[/bright_green]" if vol_multi_raw >= 2.5 else f"[dim]{vol_multi_raw:.1f}x[/dim]"
...
def run_scan_at_time_5min_fixed(time_point_aware, stocks, market_regime, is_live=False):
    ...
    for symbol, timeframe_data in stock_multi_data.items():
        ...
        if abs(score) >= Config.SCORE_THRESHOLD_MIN:
            ...
            # Calculate vol_multi: current_volume / SMA(Volume, 20)
            vol_multi = 0.0
            ...
            # Filter condition: vol_multi >= 2.5
            if vol_multi < 2.5:
                continue
...
def export_backtest_to_excel(results, filename):
    ...
    try:
        ...
        headers = ["Timestamp", "Stock", "Signal", "Score", "Action", "Volume Change", "OI Change", "Vol Multi"]
        ...
        for result in results:
            for signal in result.get('signals', []):
                ...
                ws_signals.cell(row=row_idx, column=8, value=signal.get('vol_multi', 'N/A'))
                row_idx += 1
...
def main_final_fixed():
    ...
    try:
        ...
        stocks = ["RELIANCE-EQ", "TCS-EQ", "TCS-EQ", "HDFCBANK-EQ", "INFY-EQ", "HINDUNILVR-EQ", "ICICIBANK-EQ", "SBIN-EQ"]
    ...
    if args.backtest:
        ...
        logger.info(f"Running enhanced snapshot for: {asof_ts.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        market_regime = get_market_regime(Config.BENCHMARK_INDEX)
        
        # FIXED: Use the corrected scan function
        ...
        print_colored(f"\n🎯 ENHANCED SNAPSHOT RESULTS - {asof_ts.strftime('%Y-%m-%d %H:%M')} IST", Colors.BOLD)
        print_colored(f"Market Regime: {market_regime.upper()} | Total Signals: {len(signals)}", Colors.CYAN)
        
        if GREAT_TABLES_AVAILABLE:
            create_great_table_fixed(top_bullish, "🟢 TOP 15 BULLISH OPPORTUNITIES")
            create_great_table_fixed(top_bearish, "🔴 TOP 15 BEARISH OPPORTUNITIES")
        else:
            create_rich_enhanced_table(top_bullish, "🟢 TOP 15 BULLISH OPPORTUNITIES")
            create_rich_enhanced_table(top_bearish, "🔴 TOP 15 BEARISH OPPORTUNITIES")
    
    else:
        # FIXED LIVE SCANNER with proper market timing and 5-minute boundaries
        print_colored("\n🚀 STARTING FINAL FIXED LIVE SCANNER v3.2", Colors.GREEN)
        print_colored("✅ Real OI/Volume enforced version with 5-minute precision", Colors.CYAN)
        
        if GREAT_TABLES_AVAILABLE:
            print_colored("✅ Enhanced Great-Tables visualization enabled", Colors.GREEN)
        elif RICH_AVAILABLE:
            print_colored("✅ Rich console formatting enabled", Colors.GREEN)
        else:
            print_colored("ℹ️ ASCII mode - Install rich/great-tables for enhanced display", Colors.YELLOW)
        
        print_colored("\n🔧 SCANNER FEATURES:", Colors.CYAN)
        print("  ✅ Real OpenInterest enforcement (no synthetic OI)")
        print("  ✅ 5-minute precision with proper candle boundaries")
        print("  ✅ Volume/OI % change tracking")
        print("  ✅ Multi-timeframe technical analysis")
        print("  ✅ Multi-timeframe technical analysis")
        print("  ✅ Market regime awareness")
        
        print_colored("\n📋 LIVE SCANNER USAGE:", Colors.YELLOW)
        print("  🔴 Live Trading: python scanner.py")
        print("  🔍 Snapshot: python scanner.py --asof 2025-09-30T14:25")
        print("  📈 Backtest: python scanner.py --backtest 2025-09-30")
        
        print_colored("="*100, Colors.HEADER)
        print_colored("\n🎯 Starting Live Scanner...", Colors.GREEN)
        ...
        # Wait for first scan at 09:20:15 IST
        ...
        if now_ist < first_scan_time:
            wait_seconds = (first_scan_time - now_ist).total_seconds()
            logger.info(f"Waiting {wait_seconds/60:.1f} minutes until {first_scan_time.strftime('%H:%M:%S')} IST for first scan...")
        ...
        try:
            while True:
                ...
                # Check if market is still open (15:30 IST)
                market_end_time = today_ist_dt(Config.MARKET_END)
                if now_ist.time() > market_end_time.time():
                    logger.info("📈 Market closed. Shutting down scanner.")
                    break
                
                print_colored(f"\n[{now_ist.strftime('%H:%M:%S')}] 🎯 LIVE SCANNER v3.2 - Scan #{scan_count}", Colors.HEADER)
                ...
                try:
                    ...
                    # Display results
                    scan_time_str = now_ist.strftime('%H:%M:%S')
                    total_signals = len(signals)
                    bullish_count = len([s for s in signals if s['score'] > 0])
                    bearish_count = len([s for s in signals if s['score'] < 0])
                    
                    print_colored(f"\n🎯 LIVE RESULTS - {now_ist.strftime('%Y-%m-%d %H:%M')} IST (Regime: {market_regime.upper()})", Colors.BOLD)
                    print_colored(f"📊 Total: {total_signals} | 🟢 Bullish: {bullish_count} | 🔴 Bearish: {bearish_count} | ✨ New: {len(new_stocks)}", Colors.CYAN)
                    
                    # Show volume/OI activity summary
                    vol_active = sum(1 for s in signals if isinstance(s.get('volume_change_pct', 0), (int, float)) and abs(s.get('volume_change_pct', 0)) > 1.0)
                    oi_active = sum(1 for s in signals if isinstance(s.get('oi_change_pct', 0), (int, float)) and abs(s.get('oi_change_pct', 0)) > 1.0)
                    print_colored(f"📈 Activity: {vol_active} volume spikes | {oi_active} OI changes", Colors.YELLOW)
                    
                    if top_bullish:
                        if GREAT_TABLES_AVAILABLE:
                            create_great_table_fixed(top_bullish, "🟢 TOP 15 BULLISH OPPORTUNITIES", new_stocks)
                        else:
                            create_rich_enhanced_table(top_bullish, "🟢 TOP 15 BULLISH OPPORTUNITIES", new_stocks)
                    
                    if top_bearish:
                        if GREAT_TABLES_AVAILABLE:
                            create_great_table_fixed(top_bearish, "🔴 TOP 15 BEARISH OPPORTUNITIES", new_stocks)
                        else:
                            create_rich_enhanced_table(top_bearish, "🔴 TOP 15 BEARISH OPPORTUNITIES", new_stocks)
                    
                    # Show new stocks if any
                    if new_stocks:
                        new_stocks_list = list(new_stocks)[:10]  # Show max 10
                        more_text = f" +{len(new_stocks)-10}" if len(new_stocks) > 10 else ""
                        print_colored(f"\n✨ NEW STOCKS: {', '.join(new_stocks_list)}{more_text}", Colors.MAGENTA)
                    
                    if not signals:
                        print_colored("📊 No significant signals found in current scan", Colors.YELLOW)
                    
                    # Calculate next scan time (next 5-minute boundary + settle delay)
                    ...
                    logger.info(f"⏳ Next scan at {next_scan_time.strftime('%H:%M:%S')} (waiting {wait_time_minutes:.1f} minutes)")
            ...
            print_colored("\n\n⚠️ Live scanner interrupted by user. Shutting down gracefully...", Colors.YELLOW)
        
        except Exception as e:
            logger.error(f"Critical error in live scanner: {e}")
            ...
            print_colored("✅ Cleanup complete. Scanner stopped.", Colors.GREEN)
...
if __name__ == "__main__":
    try:
        ...
        print_colored("\n" + "="*100, Colors.HEADER)
        print_colored("🎯 ENHANCED OPTION BUYER SCANNER v3.2 - REAL OI/VOLUME ENFORCED", Colors.HEADER)
        print_colored("🔗 Professional-grade technical analysis with institutional flow detection", Colors.GREEN)
        print_colored("✨ 5-minute precision | Multi-timeframe | Real OpenInterest only", Colors.CYAN)
        print_colored("="*100, Colors.HEADER)
        ...
        print(f"  📊 First Scan: {Config.FIRST_RUN_AT}:{Config.FIRST_SCAN_DELAY:02d} IST")
        print(f"  🎯 Score Threshold: {Config.SCORE_THRESHOLD_MIN}")
        print(f"  📋 Stocks File: {Config.SHARES_FILE}")
        
        # Show library status
        print_colored(f"\n📚 LIBRARIES STATUS:", Colors.CYAN)
        print(f"  ✅ Rich Console: {'Available' if RICH_AVAILABLE else 'Not Available'}")
        print(f"  ✅ Colorama: {'Available' if COLORAMA_AVAILABLE else 'Not Available'}")
        print(f"  ✅ Great Tables: {'Available' if GREAT_TABLES_AVAILABLE else 'Not Available'}")
        print(f"  ✅ Tabulate: {'Available' if TABULATE_AVAILABLE else 'Not Available'}")
        
        # Show indicator weights
        print_colored(f"\n⚖️  INDICATOR GROUPS:", Colors.CYAN)
        for group, weight in Config.GROUP_WEIGHTS.items():
            indicators_in_group = len([ind for ind in Config.INDICATOR_WEIGHTS.keys() 
                                     if ((group == 'Trend' and any(term in ind for term in ['MA', 'ADX', 'VWAP', 'EMA', 'MACD'])) or
                                         (group == 'Momentum' and any(term in ind for term in ['RSI', 'Stochastic', 'CCI', 'ROC', 'Williams'])) or
                                         (group == 'Volume' and any(term in ind for term in ['Vol', 'OBV', 'CMF'])) or
                                         (group == 'Volatility' and any(term in ind for term in ['Volatility', 'Bollinger'])) or
                                         (group == 'OI' and any(term in ind for term in ['OI', 'Option'])))])
            print(f"    {group}: Weight {weight:.1f} ({indicators_in_group} indicators)")
        
        print_colored("\n🚀 Starting Enhanced Scanner...", Colors.GREEN)
        ...
        print_colored("\n🎯 Enhanced Option Buyer Scanner v3.2 - Session Ended", Colors.HEADER)
        print_colored("📊 Thank you for using the professional trading scanner!", Colors.GREEN)
        print_colored("="*100, Colors.HEADER)

# ========== END OF COMPLETE SCANNER CODE ==========

"""
COMPLETE ENHANCED OPTION BUYER SCANNER v3.2 FEATURES:

✅ REAL OI ENFORCEMENT - Only uses actual OpenInterest data, no synthetic calculations
✅ 5-MINUTE PRECISION - Proper candle boundaries with 15-second settlement delay
✅ MULTI-TIMEFRAME ANALYSIS - 5min, 15min, 30min, 60min, 1day timeframes
✅ VOLUME/OI TRACKING - Real-time percentage changes with caching
✅ INSTITUTIONAL FLOW - Enhanced Chaikin Money Flow + Relative Volume analysis
✅ MARKET REGIME AWARE - Bullish/Bearish/Neutral regime detection with score multipliers
✅ PROFESSIONAL DISPLAY - Rich tables, Great-Tables integration, ASCII fallback
✅ LIVE + BACKTEST - Full day backtesting with time-aware data fetching
✅ ROBUST ERROR HANDLING - Rate limiting, session pooling, graceful failures
✅ CONFIGURABLE - Easy parameter tuning for different trading styles

USAGE:
  🔴 Live Trading: python scanner.py
  🔍 Snapshot: python scanner.py --asof 2025-10-03T14:25
  📈 Backtest: python scanner.py --backtest 2025-10-03

REQUIREMENTS:
  pip install pandas numpy truedata pytz tqdm rich colorama great-tables tabulate openpyxl

MARKET HOURS:
  Pre-market: 09:00-09:15 IST (waiting mode)
  Live Trading: 09:15-15:30 IST (active scanning every 5 minutes)
  Post-market: 15:30+ IST (shutdown)

SIGNAL CATEGORIES:
  📈 Very Strong Buy: Score ≥ 55
  📈 Strong Buy: Score ≥ 30
  📈 Buy Signal: Score ≥ 15
  📉 Sell Signal: Score ≤ -15
  📉 Strong Sell: Score ≤ -30
  📉 Very Strong Sell: Score ≤ -55
  ➡️  Neutral: -15 < Score < 15

TECHNICAL INDICATORS:
  Trend: MA Slope, ADX, VWAP, EMA, MACD
  Momentum: RSI, Stochastic, CCI, ROC, Williams %R
  Volume: Volume Surge, OBV, CMF, Relative Volume
  Volatility: ATR Expansion, Bollinger Bands
  OI: OI Buildup, Volume-OI Sync, Option Buyer Momentum
