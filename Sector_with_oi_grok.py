...
# Tier 1: Primary Drivers for Momentum & Confirmation (Highest Weight)
    ...
    "OISurge": 1.6,       # New: Open Interest surge for derivatives liquidity and interest.
...
class EnhancedTechnicalIndicators:
    ...
    def calculate_all_indicators(df):
        ...
        try:
            ...
            # New: OI Surge (for futures data)
            if "OI" in df.columns:
                oi = df["OI"]
                if len(df) >= 20:
                    avg20 = oi.rolling(window=20).mean()
                    current = oi
                    oir = (current / avg20.replace(0, np.nan))
                    indicators["OISurge"] = np.clip((oir - 0.5) * 40, 0, 100)
...
def normalize_indicator_value(indicator_name, value):
    try:
        if indicator_name == "RSI":
            ...
            return 50
        elif indicator_name in ("VolumeSurge", "OISurge"):
            return max(0, min(100, value))
...
class Enhanced3SectorScanner:
    ...
    def fetch_live_sectoral_performance_3sector_debug(self):
        try:
            ...
            self.worst_sectors = [sectoral_performance[-i-1]["sector"] for i in range(worst_count)]  # Corrected to get the last ones
    ...
    def fetch_live_data(self, symbol, timeframe, duration=None):
        try:
            # Use futures symbol for OI and Volume data
            futures_symbol = symbol + "-I"
            ...
            rawdf = tdhist.get_historic_data(futures_symbol, duration=duration, bar_size=bar_size)
            if rawdf is None or len(rawdf) == 0:
                return None, False

            normalized_df = self.normalize_live_data(rawdf, futures_symbol)
...
def run_backtest(self, start_date, end_date):
    try:
        ...
        if start > end:
            print(f"{Colors.RED}Error: start_date ({start_date}) is after end_date ({end_date}){Colors.RESET}")
            return
        if end > datetime.now():
            print(f"{Colors.RED}Error: end_date ({end_date}) is in the future{Colors.RESET}")
            return
        if (end - start).days < 1:
            print(f"{Colors.RED}Error: Date range is too short{Colors.RESET}")
            return

        print(f"{Colors.CYAN}{Colors.BOLD}Running backtest from {start_date} to {end_date}{Colors.RESET}")
        # Force sector update (using current sectors for backtest)
        self.force_sector_update()

        # Build target stocks same as live
        target_stocks_set = set()
        for i, sector in enumerate(self.best_sectors):
            if sector in SECTOR_STOCKS:
                alloc = [12, 10, 8, 6]
                target_stocks_set.update(SECTOR_STOCKS[sector][:alloc[i] if i < len(alloc) else 6])
        for i, sector in enumerate(self.worst_sectors):
            if sector in SECTOR_STOCKS:
                alloc = [12, 10, 8, 6]
                target_stocks_set.update(SECTOR_STOCKS[sector][:alloc[i] if i < len(alloc) else 6])
        ...
        if not target_stocks:
            print("No target stocks for backtest.")
            return

        # Calculate duration to cover start_date - lookback to now
        lookback_days = 100
        days = (end - (start - timedelta(days=lookback_days))).days + 10
        duration = f"{days} D"
        ...
        def backtest_stock(symbol):
            try:
                df, _ = self.fetch_live_data(symbol, "daily", duration=duration)
                ...
                df = df[df.index >= (start - timedelta(days=lookback_days))]

                if len(df) < lookback_days:
                    logger.warning(f"Data too short for {symbol} after filtering")
                    return None
                ...
                for i in range(lookback_days, len(df)):
                    if df.index[i] < start:
                        continue
                    past_df = df.iloc[:i+1]  # include current bar
                    timeframes_data = {"daily": past_df}
                    signal, score = self.calculate_enhanced_signals(symbol, timeframes_data)
                ...
                avg_trade_duration = np.mean([(trade["exit_date"] - trade["entry_date"]).days for trade in trades]) if trades else 0
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(backtest_stock, symbol) for symbol in target_stocks]
            for future in as_completed(futures):
        ...
        # Display results
        print(f"{Colors.GREEN}{Colors.BOLD}BACKTEST RESULTS{Colors.RESET}")
        print(f"{'Symbol':<10} {'Trades':>8} {'Total Return %':>15} {'Win Rate %':>12} {'Avg Trade Days':>15}")
        print("-" * 60)
        for res in sorted(backtest_results, key=lambda x: x["total_return"], reverse=True):
            print(f"{res['symbol']:<10} {res['num_trades']:>8} {res['total_return']:>15.2f} {res['win_rate']:>12.2f} {res['avg_trade_duration_days']:>15.2f}")
        print("-" * 60)

        # Summary statistics
        avg_return = np.mean([res["total_return"] for res in backtest_results])
        avg_win_rate = np.mean([res["win_rate"] for res in backtest_results])
        print(f"Summary: Average Return: {avg_return:.2f}%, Average Win Rate: {avg_win_rate:.2f}%")
    except Exception as e:
        print(f"{Colors.RED}Backtest failed: {e}{Colors.RESET}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced 3-Sector Scanner")
    parser.add_argument("--mode", choices=["live", "backtest"], default="live", help="Mode: live or backtest")
    parser.add_argument("--start_date", default="2024-01-01", help="Start date for backtest (YYYY-MM-DD)")
    parser.add_argument("--end_date", default="2024-12-31", help="End date for backtest (YYYY-MM-DD)")
    ...
    try:
        if args.mode == "live":
            ...
            scanner.run_backtest(args.start_date, args.end_date)
    except KeyboardInterrupt:
